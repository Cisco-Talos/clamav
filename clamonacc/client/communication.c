...
int onas_sendln(CURL *curl, const void *line, size_t len, int64_t timeout)
{
    ...
    if (CURLE_OK != curlcode) {
        logg(ERROR, "ClamCom: could not get curl active socket info %s\n", curl_easy_strerror(curlcode));
        return 1;
    }
    ...
    while (len) {
        ...
        do {
            ...
            if (CURLE_AGAIN == curlcode && onas_socket_wait(sockfd, 0, timeout) <= 0) {
                logg(ERROR, "ClamCom: TIMEOUT while waiting on socket (send)\n");
                return 1;
            }
        ...
        if (sent == 0) {
            if (sent && errno == EINTR) {
                ...
                /* Users have reported frequent "bad address" errors when files
                   ...
                   verbose-mode only. */
                logg(DEBUG, "Can't send to clamd: %s\n", strerror(errno));
            } else {
        ...
        if (!rcv_data->retlen) {
            do {
                ...
                if (CURLE_AGAIN == rcv_data->curlcode && onas_socket_wait(sockfd, 1, timeout) <= 0) {
                    logg(ERROR, "ClamCom: TIMEOUT while waiting on socket (recv)\n");
                    return -1;
                }
                ...
                if (rcv_data->retlen || rcv_data->curr != rcv_data->buf) {
                    *rcv_data->curr = '\0';
                    ...
                    if (strcmp(rcv_data->buf, "UNKNOWN COMMAND\n")) {
                        logg(ERROR, "Communication error, clamd received unknown command\n");
                    } else {
