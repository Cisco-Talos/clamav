...
void *onas_scan_queue_th(void *arg)
{
    ...
    logg(DEBUG, "ClamScanQueue: initializing event queue consumer ... (%d) threads in thread pool\n", ctx->maxthreads);
    ...
    logg(DEBUG, "ClamScanQueue: waiting to consume events ...\n");
    do {
...
cl_error_t onas_scan_queue_start(struct onas_context **ctx)
{
    ...
    if (!ctx || !*ctx) {
        logg(DEBUG, "ClamScanQueue: unable to start clamonacc. (bad context)\n");
        return CL_EARG;
    }
    ...
    if (0 != thread_started) {
        ...
        logg(DEBUG, "ClamScanQueue: Unable to start event consumer queue thread ... \n");
        return CL_ECREAT;
    }
...
static void onas_scan_queue_exit(void *arg)
{
    ...
    logg(DEBUG, "ClamScanQueue: onas_scan_queue_exit()\n");
    if (g_thpool) {
    ...
    logg(INFO, "ClamScanQueue: stopped\n");
}
