...
static int hexdump(void)
{
    ...
    while ((bytes = read(0, buffer, FILEBUFF)) > 0) {
        ...
        if (write(1, pt, 2 * bytes) == -1) {
            mprintf(ERROR, "hexdump: Can't write to stdout\n");
            free(pt);
            return -1;
        }
        free(pt);
...
static int hashpe(const char *filename, unsigned int class, int type)
{
    ...
    if (fd < 0) {
        mprintf(ERROR, "hashpe: Can't open file %s!\n", filename);
        goto done;
    }
    ...
    if (NULL == new_map) {
        mprintf(ERROR, "hashpe: Can't create fmap for open file\n");
        goto done;
    }

    /* build engine */
    if (!(engine = cl_engine_new())) {
        mprintf(ERROR, "hashpe: Can't create new engine\n");
        goto done;
    }
    cl_engine_set_num(engine, CL_ENGINE_AC_ONLY, 1);

    if (cli_initroots(engine, 0) != CL_SUCCESS) {
        mprintf(ERROR, "hashpe: cli_initroots() failed\n");
        goto done;
    }

    if (cli_parse_add(engine->root[0], "test", "deadbeef", 0, 0, 0, "*", 0, NULL, 0) != CL_SUCCESS) {
        mprintf(ERROR, "hashpe: Can't parse signature\n");
        goto done;
    }

    if (cl_engine_compile(engine) != CL_SUCCESS) {
        mprintf(ERROR, "hashpe: Can't compile engine\n");
        goto done;
    }
    ...
    if (!fmptr) {
        mprintf(ERROR, "hashpe: fmap_need_off_once failed!\n");
        goto done;
    }
    ...
    switch (class) {
        ...
        case 2:
            ...
            break;
        default:
            mprintf(ERROR, "hashpe: unknown classification(%u) for pe hash!\n", class);
            goto done;
    }
    ...
    switch (ret) {
        case CL_CLEAN:
            break;
        case CL_VIRUS:
            mprintf(DEBUG, "hashpe: CL_VIRUS after cli_genhash_pe()!\n");
            break;
        case CL_BREAK:
            mprintf(DEBUG, "hashpe: CL_BREAK after cli_genhash_pe()!\n");
            break;
        case CL_EFORMAT:
            mprintf(ERROR, "hashpe: Not a valid PE file!\n");
            break;
        default:
            mprintf(ERROR, "hashpe: Other error %d inside cli_genhash_pe.\n", ret);
            break;
    }
    ...
    if (opts->filename) {
        for (i = 0; opts->filename[i]; i++) {
            if (CLAMSTAT(opts->filename[i], &sb) == -1) {
                perror("hashsig");
                mprintf(ERROR, "hashsig: Can't access file %s\n", opts->filename[i]);
                return -1;
            } else {
                if ((sb.st_mode & S_IFMT) == S_IFREG) {
                    if ((class == 0) && (hash = cli_hashfile(opts->filename[i], type))) {
                        ...
                        mprintf(ERROR, "hashsig: Can't generate hash for %s\n", opts->filename[i]);
                        return -1;
                    }
                }
            }
        ...
        if (class > 0) {
            mprintf(ERROR, "hashsig: Can't generate requested hash for input stream\n");
            return -1;
        }
        hash = cli_hashstream(stdin, NULL, type);
        if (!hash) {
            mprintf(ERROR, "hashsig: Can't generate hash for input stream\n");
            return -1;
        }
        mprintf(INFO, "%s\n", hash);
        free(hash);
...
static int htmlnorm(const struct optstruct *opts)
{
    ...
    if ((fd = open(optget(opts, "html-normalise")->strarg, O_RDONLY | O_BINARY)) == -1) {
        mprintf(ERROR, "htmlnorm: Can't open file %s\n", optget(opts, "html-normalise")->strarg);
        return -1;
    }
...
static int asciinorm(const struct optstruct *opts)
{
    ...
    if (fd == -1) {
        mprintf(ERROR, "asciinorm: Can't open file %s\n", fname);
        return -1;
    }

    if (!(norm_buff = malloc(ASCII_FILE_BUFF_LENGTH))) {
        mprintf(ERROR, "asciinorm: Can't allocate memory\n");
        close(fd);
        return -1;
    }

    if (!(map = fmap(fd, 0, 0, fname))) {
        mprintf(ERROR, "fmap: Could not map fd %d\n", fd);
        close(fd);
        free(norm_buff);
        return -1;
    }
    ...
    if (map->len > MAX_ASCII_FILE_SIZE) {
        mprintf(ERROR, "asciinorm: File size of %zu too large\n", map->len);
        close(fd);
        free(norm_buff);
        funmap(map);
        return -1;
    ...
    if (ofd == -1) {
        mprintf(ERROR, "asciinorm: Can't open file ./normalised_text\n");
        close(fd);
        free(norm_buff);
        funmap(map);
        return -1;
    ...
    while (map_off != map->len) {
        ...
        if (write(ofd, norm_buff, state.out_pos) == -1) {
            mprintf(ERROR, "asciinorm: Can't write to file ./normalised_text\n");
            close(fd);
            close(ofd);
            free(norm_buff);
            funmap(map);
...
static int utf16decode(const struct optstruct *opts)
{
    ...
    if ((fd1 = open(fname, O_RDONLY | O_BINARY)) == -1) {
        mprintf(ERROR, "utf16decode: Can't open file %s\n", fname);
        return -1;
    }

    newname = malloc(strlen(fname) + 7);
    if (!newname) {
        mprintf(ERROR, "utf16decode: Can't allocate memory\n");
        close(fd1);
        return -1;
    }
    ...
    if ((fd2 = open(newname, O_WRONLY | O_CREAT | O_TRUNC | O_BINARY, S_IRUSR | S_IWUSR)) < 0) {
        mprintf(ERROR, "utf16decode: Can't create file %s\n", newname);
        free(newname);
        close(fd1);
        return -1;
    }
    ...
    while ((bytes = read(fd1, buff, sizeof(buff))) > 0) {
        ...
        if (decoded) {
            if (write(fd2, decoded, strlen(decoded)) == -1) {
                mprintf(ERROR, "utf16decode: Can't write to file %s\n", newname);
                free(decoded);
                close(fd1);
                close(fd2);
                unlink(newname);
...
static char *sha256file(const char *file, unsigned int *size)
{
    ...
    if (!(fh = fopen(file, "rb"))) {
        mprintf(ERROR, "sha256file: Can't open file %s\n", file);
        cl_hash_destroy(ctx);
        return NULL;
    }
    if (size)
...
static int writeinfo(const char *dbname, const char *builder, const char *header, const struct optstruct *opts, char *const *dblist2, unsigned int dblist2cnt)
{
    ...
    if (!access(file, R_OK)) {
        if (unlink(file) == -1) {
            mprintf(ERROR, "writeinfo: Can't unlink %s\n", file);
            return -1;
        }
    }

    if (!(fh = fopen(file, "wb+"))) {
        mprintf(ERROR, "writeinfo: Can't create file %s\n", file);
        return -1;
    }

    if (fprintf(fh, "%s\n", header) < 0) {
        mprintf(ERROR, "writeinfo: Can't write to %s\n", file);
        fclose(fh);
        return -1;
    }
    ...
    if (dblist2cnt) {
        for (i = 0; i < dblist2cnt; i++) {
            if (!(pt = sha256file(dblist2[i], &bytes))) {
                mprintf(ERROR, "writeinfo: Can't generate SHA256 for %s\n", file);
                fclose(fh);
                return -1;
            }
            if (fprintf(fh, "%s:%u:%s\n", dblist2[i], bytes, pt) < 0) {
                mprintf(ERROR, "writeinfo: Can't write to info file\n");
                fclose(fh);
                free(pt);
                return -1;
            }
    ...
    if (!optget(opts, "unsigned")->enabled) {
        ...
        if (!(pt = cli_getdsig(optget(opts, "server")->strarg, builder, digest, 32, 3))) {
            mprintf(ERROR, "writeinfo: Can't get digital signature from remote server\n");
            fclose(fh);
            return -1;
        }
        fprintf(fh, "DSIG:%s\n", pt);
...
#define FREE_LS(x)                   \
    ...
    if (!optget(opts, "server")->enabled && !optget(opts, "unsigned")->enabled) {
        mprintf(ERROR, "build: --server is required for --build\n");
        return -1;
    }
    ...
    if (CLAMSTAT("COPYING", &foo) == -1) {
        mprintf(ERROR, "build: COPYING file not found in current working directory.\n");
        return -1;
    }
    ...
    if (!(engine = cl_engine_new())) {
        mprintf(ERROR, "build: Can't initialize antivirus engine\n");
        return 50;
    }

    if ((ret = cl_load(".", engine, &sigs, CL_DB_STDOPT | CL_DB_PUA | CL_DB_SIGNED))) {
        mprintf(ERROR, "build: Can't load database: %s\n", cl_strerror(ret));
        cl_engine_free(engine);
        return -1;
    }
    cl_engine_free(engine);
    ...
    if (!sigs) {
        mprintf(ERROR, "build: There are no signatures in database files\n");
    } else {
        if (bc || hy) {
            if ((dd = opendir(".")) == NULL) {
                mprintf(ERROR, "build: Can't open current directory\n");
                return -1;
            }
                ...
                if (dent->d_ino) {
                    if (cli_strbcasestr(dent->d_name, ".cbc")) {
                        ...
                        if (!dblist2) { /* dblist2 leaked but we don't really care */
                            mprintf(ERROR, "build: Memory allocation error\n");
                            closedir(dd);
                            return -1;
                        }
            ...
            if (!access("last.hdb", R_OK)) {
                if (!dblist2cnt) {
                    mprintf(ERROR, "build: dblist2 == NULL (no .cbc files?)\n");
                    return -1;
                }
                dblist2 = (char **)realloc(dblist2, (dblist2cnt + 1) * sizeof(char *));
                if (!dblist2) {
        ...
        if (entries != sigs)
            mprintf(WARNING, "build: Signatures in %s db files: %u, loaded by libclamav: %u\n", dbname, entries, sigs);

        maxentries = optget(opts, "max-bad-sigs")->numarg;

        if (maxentries) {
            if (!entries || (sigs > entries && sigs - entries >= maxentries)) {
                mprintf(ERROR, "Bad number of signatures in database files\n");
                FREE_LS(dblist2);
                return -1;
            }
        }
    ...
    if (opts->filename) {
        if (cli_strbcasestr(opts->filename[0], ".cvd") || cli_strbcasestr(opts->filename[0], ".cld") || cli_strbcasestr(opts->filename[0], ".cud")) {
            ...
            mprintf(ERROR, "build: Not a CVD/CLD/CUD file\n");
            FREE_LS(dblist2);
            return -1;
        }
    ...
    if (!(oldcvd = cl_cvdhead(olddb)) && !optget(opts, "unsigned")->enabled) {
        mprintf(WARNING, "build: CAN'T READ CVD HEADER OF CURRENT DATABASE %s (wait 3 s)\n", olddb);
        sleep(3);
    }
    ...
    if (oldcvd) {
        ...
        mprintf(INFO, "Version number: ");
        if (scanf("%u", &version) == EOF) {
            mprintf(ERROR, "build: scanf() failed\n");
            FREE_LS(dblist2);
            return -1;
        }
    }

    mprintf(INFO, "Total sigs: %u\n", sigs);
    if (sigs > oldsigs)
        mprintf(INFO, "New sigs: %u\n", sigs - oldsigs);
    ...
    if ((pt = getenv("SIGNDUSER"))) {
        ...
        mprintf(INFO, "Builder name: ");
        if (scanf("%32s", builder) == EOF) {
            mprintf(ERROR, "build: Can't get builder name\n");
            free(dblist2);
            return -1;
        }
    }
    ...
    if (writeinfo(dbname, builder, header, opts, dblist2, dblist2cnt) == -1) {
        mprintf(ERROR, "build: Can't generate info file\n");
        FREE_LS(dblist2);
        return -1;
    }
    ...
    if (!(tarfile = cli_gentemp("."))) {
        mprintf(ERROR, "build: Can't generate temporary name for tarfile\n");
        FREE_LS(dblist2);
        return -1;
    }

    if ((tar = gzopen(tarfile, "wb9f")) == NULL) {
        mprintf(ERROR, "build: Can't open file %s for writing\n", tarfile);
        free(tarfile);
        FREE_LS(dblist2);
        return -1;
    }
    ...
    if (tar_addfile(-1, tar, "COPYING") == -1) {
        mprintf(ERROR, "build: Can't add COPYING to tar archive\n");
        gzclose(tar);
        unlink(tarfile);
        free(tarfile);
        FREE_LS(dblist2);
    ...
    if (!(fh = fopen(tarfile, "rb"))) {
        mprintf(ERROR, "build: Can't open file %s for reading\n", tarfile);
        unlink(tarfile);
        free(tarfile);
        return -1;
    }
    ...
    if (!(pt = cli_hashstream(fh, buffer, 1))) {
        mprintf(ERROR, "build: Can't generate MD5 checksum for %s\n", tarfile);
        fclose(fh);
        unlink(tarfile);
        free(tarfile);
        return -1;
    ...
    if (!optget(opts, "unsigned")->enabled) {
        if (!(pt = cli_getdsig(optget(opts, "server")->strarg, builder, buffer, 16, 1))) {
            mprintf(ERROR, "build: Can't get digital signature from remote server\n");
            fclose(fh);
            unlink(tarfile);
            free(tarfile);
            return -1;
    ...
    if (!(cvd = fopen(newcvd, "wb"))) {
        mprintf(ERROR, "build: Can't create final database %s\n", newcvd);
        fclose(fh);
        unlink(tarfile);
        free(tarfile);
        return -1;
    ...
    if (fwrite(header, 1, 512, cvd) != 512) {
        mprintf(ERROR, "build: Can't write to %s\n", newcvd);
        fclose(fh);
        unlink(tarfile);
        free(tarfile);
        fclose(cvd);
    ...
    if (unlink(tarfile) == -1) {
        mprintf(WARNING, "build: Can't unlink %s\n", tarfile);
        unlink(tarfile);
        free(tarfile);
        unlink(newcvd);
        return -1;
    ...
    mprintf(INFO, "Created %s\n", newcvd);

    if (optget(opts, "unsigned")->enabled)
        return 0;

    if (!oldcvd || optget(opts, "no-cdiff")->enabled) {
        mprintf(INFO, "Skipping .cdiff creation\n");
        return 0;
    }

    /* generate patch */
    if (!(pt = cli_gentemp(NULL))) {
        mprintf(ERROR, "build: Can't generate temporary name\n");
        unlink(newcvd);
        return -1;
    }

    if (mkdir(pt, 0700)) {
        mprintf(ERROR, "build: Can't create temporary directory %s\n", pt);
        free(pt);
        unlink(newcvd);
        return -1;
    }
    ...
    if (cli_cvdunpack(olddb, pt) == -1) {
        mprintf(ERROR, "build: Can't unpack CVD file %s\n", olddb);
        cli_rmdirs(pt);
        free(pt);
        unlink(newcvd);
        return -1;
    ...
    if (ret == -1) {
        ...
        if (rename(patch, broken)) {
            unlink(patch);
            mprintf(ERROR, "Generated file is incorrect, removed");
        } else {
            mprintf(ERROR, "Generated file is incorrect, renamed to %s\n", broken);
        }
    } else {
...
static int unpack(const struct optstruct *opts)
{
    ...
    if (cl_cvdverify(name) != CL_SUCCESS) {
        mprintf(ERROR, "unpack: %s is not a valid CVD\n", name);
        return -1;
    }

    if (cli_cvdunpack(name, ".") == -1) {
        mprintf(ERROR, "unpack: Can't unpack file %s\n", name);
        return -1;
    }
...
static int cvdinfo(const struct optstruct *opts)
{
    ...
    if ((cvd = cl_cvdhead(pt)) == NULL) {
        mprintf(ERROR, "cvdinfo: Can't read/parse CVD header of %s\n", pt);
        return -1;
    }
    mprintf(INFO, "File: %s\n", pt);
    ...
    *pt = ':';
    mprintf(INFO, "Build time: %s\n", cvd->time);
    mprintf(INFO, "Version: %u\n", cvd->version);
    mprintf(INFO, "Signatures: %u\n", cvd->sigs);
    mprintf(INFO, "Functionality level: %u\n", cvd->fl);
    mprintf(INFO, "Builder: %s\n", cvd->builder);

    pt = optget(opts, "info")->strarg;
    if (cli_strbcasestr(pt, ".cvd")) {
        mprintf(INFO, "MD5: %s\n", cvd->md5);
        mprintf(INFO, "Digital signature: %s\n", cvd->dsig);
    }
    cl_cvdfree(cvd);
    if (cli_strbcasestr(pt, ".cud"))
        mprintf(INFO, "Verification: Unsigned container\n");
    else if ((ret = cl_cvdverify(pt))) {
        mprintf(ERROR, "cvdinfo: Verification: %s\n", cl_strerror(ret));
        return -1;
    } else
        mprintf(INFO, "Verification OK.\n");
...
static int listdir(const char *dirname, const regex_t *regex)
{
    ...
    if ((dd = opendir(dirname)) == NULL) {
        mprintf(ERROR, "listdir: Can't open directory %s\n", dirname);
        return -1;
    }
    ...
    while ((dent = readdir(dd))) {
        if (dent->d_ino) {
            if (strcmp(dent->d_name, ".") && strcmp(dent->d_name, "..") &&
                ...
                if (!dbfile) {
                    mprintf(ERROR, "listdir: Can't allocate memory for dbfile\n");
                    closedir(dd);
                    return -1;
                }
                ...
                if (listdb(dbfile, regex) == -1) {
                    mprintf(ERROR, "listdb: Error listing database %s\n", dbfile);
                    free(dbfile);
                    closedir(dd);
                    return -1;
                }
...
static int listdb(const char *filename, const regex_t *regex)
{
    ...
    if ((fh = fopen(filename, "rb")) == NULL) {
        mprintf(ERROR, "listdb: Can't open file %s\n", filename);
        return -1;
    }

    if (!(buffer = (char *)malloc(CLI_DEFAULT_LSIG_BUFSIZE + 1))) {
        mprintf(ERROR, "listdb: Can't allocate memory for buffer\n");
        fclose(fh);
        return -1;
    }
    ...
    if (!fgets(buffer, 12, fh)) {
        mprintf(ERROR, "listdb: fgets failed\n");
        free(buffer);
        fclose(fh);
        return -1;
    }
    ...
    if (!strncmp(buffer, "ClamAV-VDB:", 11)) {
        ...
        if (!(dir = cli_gentemp(NULL))) {
            mprintf(ERROR, "listdb: Can't generate temporary name\n");
            return -1;
        }

        if (mkdir(dir, 0700)) {
            mprintf(ERROR, "listdb: Can't create temporary directory %s\n", dir);
            free(dir);
            return -1;
        }

        if (cli_cvdunpack(filename, dir) == -1) {
            mprintf(ERROR, "listdb: Can't unpack CVD file %s\n", filename);
            cli_rmdirs(dir);
            free(dir);
            return -1;
        }
        ...
        if (listdir(dir, regex) == -1) {
            mprintf(ERROR, "listdb: Can't list directory %s\n", filename);
            cli_rmdirs(dir);
            free(dir);
            return -1;
        }
    ...
    if (!(dbname = strrchr(filename, *pathsep))) {
        mprintf(ERROR, "listdb: Invalid filename %s\n", filename);
        fclose(fh);
        free(buffer);
        return -1;
    }
    ...
    if (cli_strbcasestr(filename, ".db")) { /* old style database */
        ...
        while (fgets(buffer, CLI_DEFAULT_LSIG_BUFSIZE, fh)) {
            ...
            pt = strchr(buffer, '=');
            if (!pt) {
                mprintf(ERROR, "listdb: Malformed pattern line %u (file %s)\n", line, filename);
                fclose(fh);
                free(buffer);
                return -1;
            }
        ...
        if (fgets(buffer, CLI_DEFAULT_LSIG_BUFSIZE, fh) && fgets(buffer, CLI_DEFAULT_LSIG_BUFSIZE, fh)) {
            ...
            if (regex) {
                if (!cli_regexec(regex, buffer, 0, NULL, 0)) {
                    mprintf(INFO, "[%s BYTECODE] %s", dbname, buffer);
                }
            } else {
                *pt = 0;
                mprintf(INFO, "%s\n", buffer);
...
static int listsigs(const struct optstruct *opts, int mode)
{
    ...
    if (mode == 0) {
        ...
        if (CLAMSTAT(name, &sb) == -1) {
            mprintf(INFO, "--list-sigs: Can't get status of %s\n", name);
            return -1;
        }
        ...
        if (cli_regcomp(&reg, optget(opts, "find-sigs")->strarg, REG_EXTENDED | REG_NOSUB) != 0) {
            mprintf(INFO, "--find-sigs: Can't compile regex\n");
            return -1;
        }
        mprintf_stdout = 1;
        dbdir          = freshdbdir();
...
static int vbadump(const struct optstruct *opts)
{
    ...
    if ((fd = open(pt, O_RDONLY | O_BINARY)) == -1) {
        mprintf(ERROR, "vbadump: Can't open file %s\n", pt);
        return -1;
    }

    /* generate the temporary directory */
    if (!(dir = cli_gentemp(NULL))) {
        mprintf(ERROR, "vbadump: Can't generate temporary name\n");
        close(fd);
        return -1;
    }

    if (mkdir(dir, 0700)) {
        mprintf(ERROR, "vbadump: Can't create temporary directory %s\n", dir);
        free(dir);
        close(fd);
        return -1;
    }
...
static int comparesha(const char *diff)
{
    ...
    name = strdup(diff);
    if (!name) {
        mprintf(ERROR, "verifydiff: strdup() failed\n");
        return -1;
    }
    if (!(pt = strrchr(name, '-')) || !isdigit(pt[1])) {
        mprintf(ERROR, "verifydiff: Invalid diff name\n");
        free(name);
        return -1;
    }
    *pt = 0;
    ...
    if (!(fh = fopen(info, "rb"))) {
        mprintf(ERROR, "verifydiff: Can't open %s\n", info);
        return -1;
    }

    if (!fgets(buff, sizeof(buff), fh) || strncmp(buff, "ClamAV-VDB", 10)) {
        mprintf(ERROR, "verifydiff: Incorrect info file %s\n", info);
        fclose(fh);
        return -1;
    }
    ...
    while (fgets(buff, sizeof(buff), fh)) {
        ...
        if (tokens_count != 3) {
            if (!strcmp(tokens[0], "DSIG"))
                continue;
            mprintf(ERROR, "verifydiff: Incorrect format of %s\n", info);
            ret = -1;
            break;
        }
        if (!(sha = sha256file(tokens[0], NULL))) {
            mprintf(ERROR, "verifydiff: Can't generate SHA256 for %s\n", buff);
            ret = -1;
            break;
        }
        if (strcmp(sha, tokens[2])) {
            mprintf(ERROR, "verifydiff: %s has incorrect checksum\n", buff);
            ret = -1;
            free(sha);
            break;
        }
...
static int rundiff(const struct optstruct *opts)
{
    ...
    if (strstr(diff, ".cdiff")) {
        ...
        mprintf(ERROR, "rundiff: Incorrect file name (no .cdiff/.script extension)\n");
        return -1;
    }

    if ((fd = open(diff, O_RDONLY | O_BINARY)) == -1) {
        mprintf(ERROR, "rundiff: Can't open file %s\n", diff);
        return -1;
    }
...
static int maxlinelen(const char *file)
{
    ...
    if ((fd = open(file, O_RDONLY | O_BINARY)) == -1) {
        mprintf(ERROR, "maxlinelen: Can't open file %s\n", file);
        return -1;
    }
    ...
    if (bytes == -1) {
        mprintf(ERROR, "maxlinelen: Can't read file %s\n", file);
        close(fd);
        return -1;
    }
...
static int compare(const char *oldpath, const char *newpath, FILE *diff)
{
    ...
    if (!access(oldpath, R_OK) && (omd5 = cli_hashfile(oldpath, 1))) {
        if (!(nmd5 = cli_hashfile(newpath, 1))) {
            mprintf(ERROR, "compare: Can't get MD5 checksum of %s\n", newpath);
            free(omd5);
            return -1;
        }
        if (!strcmp(omd5, nmd5)) {
    ...
    obuff = malloc(l1);
    if (!obuff) {
        mprintf(ERROR, "compare: Can't allocate memory for 'obuff'\n");
        return -1;
    }
    nbuff = malloc(l1);
    if (!nbuff) {
        mprintf(ERROR, "compare: Can't allocate memory for 'nbuff'\n");
        free(obuff);
        return -1;
    }
    tbuff = malloc(l1);
    if (!tbuff) {
        mprintf(ERROR, "compare: Can't allocate memory for 'tbuff'\n");
        free(obuff);
        free(nbuff);
        return -1;
    }
    ...
    if (!(new = fopen(newpath, "rb"))) {
        mprintf(ERROR, "compare: Can't open file %s for reading\n", newpath);
        free(obuff);
        free(nbuff);
        free(tbuff);
        return -1;
    ...
    while (fgets(nbuff, l1, new)) {
        ...
        if (i >= 2 && (nbuff[i - 1] == '\r' || (nbuff[i - 1] == '\n' && nbuff[i - 2] == '\r'))) {
            mprintf(ERROR, "compare: New %s file contains lines terminated with CRLF or CR\n", newpath);
            if (old)
                fclose(old);
            fclose(new);
            free(obuff);
...
{
    if (!opts->filename) {
        mprintf(ERROR, "makediff: --compare requires two arguments\n");
        return -1;
    }
...
static int dircopy(const char *src, const char *dest)
{
    ...
    if (CLAMSTAT(dest, &sb) == -1) {
        if (mkdir(dest, 0755)) {
            /* mprintf(ERROR, "dircopy: Can't create temporary directory %s\n", dest); */
            return -1;
        }
    }

    if ((dd = opendir(src)) == NULL) {
        /* mprintf(ERROR, "dircopy: Can't open directory %s\n", src); */
        return -1;
    }
    ...
    while ((dent = readdir(dd))) {
        if (dent->d_ino) {
            ...
            if (filecopy(spath, dpath) == -1) {
                /* mprintf(ERROR, "dircopy: Can't copy %s to %s\n", spath, dpath); */
                cli_rmdirs(dest);
                closedir(dd);
                return -1;
            }
...
static int verifydiff(const char *diff, const char *cvd, const char *incdir)
{
    ...
    if (strstr(diff, ".cdiff")) {
        ...
        mprintf(ERROR, "verifydiff: Incorrect file name (no .cdiff/.script extension)\n");
        return -1;
    }

    tempdir = cli_gentemp(NULL);
    if (!tempdir) {
        mprintf(ERROR, "verifydiff: Can't generate temporary name for tempdir\n");
        return -1;
    }

    if (mkdir(tempdir, 0700) == -1) {
        mprintf(ERROR, "verifydiff: Can't create directory %s\n", tempdir);
        free(tempdir);
        return -1;
    }
    ...
    if (cvd) {
        if (cli_cvdunpack(cvd, tempdir) == -1) {
            mprintf(ERROR, "verifydiff: Can't unpack CVD file %s\n", cvd);
            cli_rmdirs(tempdir);
            free(tempdir);
            return -1;
        }
        ...
        if (dircopy(incdir, tempdir) == -1) {
            mprintf(ERROR, "verifydiff: Can't copy dir %s to %s\n", incdir, tempdir);
            cli_rmdirs(tempdir);
            free(tempdir);
            return -1;
        }
    ...
    if (!getcwd(cwd, sizeof(cwd))) {
        mprintf(ERROR, "verifydiff: getcwd() failed\n");
        cli_rmdirs(tempdir);
        free(tempdir);
        return -1;
    }
    ...
    if ((fd = open(diff, O_RDONLY | O_BINARY)) == -1) {
        mprintf(ERROR, "verifydiff: Can't open diff file %s\n", diff);
        cli_rmdirs(tempdir);
        free(tempdir);
        return -1;
    }
    ...
    if (chdir(tempdir) == -1) {
        mprintf(ERROR, "verifydiff: Can't chdir to %s\n", tempdir);
        cli_rmdirs(tempdir);
        free(tempdir);
        close(fd);
        return -1;
    ...
    if (cdiff_apply(fd, mode) == -1) {
        mprintf(ERROR, "verifydiff: Can't apply %s\n", diff);
        if (chdir(cwd) == -1)
            mprintf(WARNING, "verifydiff: Can't chdir to %s\n", cwd);
        cli_rmdirs(tempdir);
        free(tempdir);
        close(fd);
        return -1;
    ...
    if (!ret) {
        if (cvd)
            mprintf(INFO, "Verification: %s correctly applies to %s\n", diff, cvd);
        else
            mprintf(INFO, "Verification: %s correctly applies to the previous version\n", diff);
    }
...
static void matchsig(const char *sig, const char *offset, int fd)
{
    ...
    /* build engine */
    if (!(engine = cl_engine_new())) {
        mprintf(ERROR, "matchsig: Can't create new engine\n");
        goto done;
    }
    cl_engine_set_num(engine, CL_ENGINE_AC_ONLY, 1);

    if (cli_initroots(engine, 0) != CL_SUCCESS) {
        mprintf(ERROR, "matchsig: cli_initroots() failed\n");
        goto done;
    }

    if (cli_parse_add(engine->root[0], "test", sig, 0, 0, 0, "*", 0, NULL, 0) != CL_SUCCESS) {
        mprintf(ERROR, "matchsig: Can't parse signature\n");
        goto done;
    }

    if (cl_engine_compile(engine) != CL_SUCCESS) {
        mprintf(ERROR, "matchsig: Can't compile engine\n");
        goto done;
    }
    ...
    if (matches) {
        ...
        mprintf(INFO, "MATCH: ** YES%s ** (%u %s:", offset ? "/CHECK OFFSET" : "", matches, matches > 1 ? "matches at offsets" : "match at offset");
        res = acres;
        while (res) {
            mprintf(INFO, " %u", (unsigned int)res->offset);
            res = res->next;
        }
        mprintf(INFO, ")\n");
    } else {
        mprintf(INFO, "MATCH: ** NO **\n");
    }
...
static char *decodehexstr(const char *hex, unsigned int *dlen)
{
    ...
    if (!decoded) {
        free(str16);
        mprintf(ERROR, "decodehexstr: Can't allocate memory for decoded\n");
        return NULL;
    }
    ...
    for (i = 0; i < len; i++) {
        if (str16[i] & CLI_MATCH_WILDCARD) {
            switch (str16[i] & CLI_MATCH_WILDCARD) {
                ...
                default:
                    mprintf(ERROR, "decodehexstr: Unknown wildcard (0x%x@%u)\n", str16[i] & CLI_MATCH_WILDCARD, i);
                    free(decoded);
                    free(str16);
                    return NULL;
            }
...
static char *decodehexspecial(const char *hex, unsigned int *dlen)
{
    ...
    hexcpy = strdup(hex);
    if (!hexcpy) {
        mprintf(ERROR, "decodehexspecial: strdup(hex) failed\n");
        return NULL;
    }
    pt = strchr(hexcpy, '(');
    if (!pt) {
        ...
        if (!buff) {
            mprintf(ERROR, "decodehexspecial: Can't allocate memory for buff\n");
            free(hexcpy);
            return NULL;
        }
        start = hexcpy;
        do {
            ...
            *pt++    = 0;
            if (!start) {
                mprintf(ERROR, "decodehexspecial: Unexpected EOL\n");
                free(hexcpy);
                free(buff);
                return NULL;
            }
            ...
            if (!(decoded = decodehexstr(start, &hlen))) {
                mprintf(ERROR, "Decoding failed (1): %s\n", pt);
                free(hexcpy);
                free(buff);
                return NULL;
            }
            ...
            if (!(start = get_paren_end(pt))) {
                mprintf(ERROR, "decodehexspecial: Missing closing parenthesis\n");
                free(hexcpy);
                free(buff);
                return NULL;
            }
            ...
            *start++ = 0;
            if (!strlen(pt)) {
                mprintf(ERROR, "decodehexspecial: Empty block\n");
                free(hexcpy);
                free(buff);
                return NULL;
            }
            ...
            if (!strcmp(pt, "B")) {
                ...
                while ((level >= 0) && (e = strpbrk(h, "()|"))) {
                    ...
                    *e++ = 0;
                    if (op != '(' && lop != ')' && !strlen(h)) {
                        mprintf(ERROR, "decodehexspecial: Empty string alternative block\n");
                        free(hexcpy);
                        free(buff);
                        return NULL;
                    }

                    //mprintf(INFO, "decodehexspecial: %s\n", h);
                    if (!(c = cli_hex2str(h))) {
                        mprintf(ERROR, "Decoding failed (3): %s\n", h);
                        free(hexcpy);
                        free(buff);
                        return NULL;
                    }
                ...
                if (!(c = cli_hex2str(h))) {
                    mprintf(ERROR, "Decoding failed (4): %s\n", h);
                    free(hexcpy);
                    free(buff);
                    return NULL;
                }
                ...
                buff[len++] = '}';
                if (level != 0) {
                    mprintf(ERROR, "decodehexspecial: Invalid string alternative nesting\n");
                    free(hexcpy);
                    free(buff);
                    return NULL;
                }
        ...
        if (start) {
            if (!(decoded = decodehexstr(start, &hlen))) {
                mprintf(ERROR, "Decoding failed (2)\n");
                free(buff);
                free(hexcpy);
                return NULL;
            }
...
static int decodehex(const char *hexsig)
{
    ...
    if ((wild = strchr(hexsig, '/'))) {
        ...
        /* check for trigger */
        if (!tlen) {
            mprintf(ERROR, "pcre without logical trigger\n");
            return -1;
        }

        /* locate end of regex for options start, locate options length */
        if ((regex_end = strchr(wild + 1, '/')) == NULL) {
            mprintf(ERROR, "missing regex expression terminator /\n");
            return -1;
        }
        ...
        if (!trigger) {
            mprintf(ERROR, "cannot allocate memory for trigger string\n");
            return -1;
        }
        ...
        if (!regex) {
            mprintf(ERROR, "cannot allocate memory for regex expression\n");
            free(trigger);
            return -1;
        }
        ...
        if (clen) {
            ...
            if (!cflags) {
                mprintf(ERROR, "cannot allocate memory for compile flags\n");
                free(trigger);
                free(regex);
                return -1;
            }
        ...
        mprintf(INFO, "     +-> TRIGGER: %s\n", trigger);
        mprintf(INFO, "     +-> REGEX: %s\n", regex);
        mprintf(INFO, "     +-> CFLAGS: %s\n", cflags);
        ...
        return 0;
#else
        mprintf(ERROR, "PCRE subsig cannot be loaded without PCRE support\n");
        return -1;
#endif
        ...
        for (i = 1; i <= parts; i++) {
            ...
            if (mindist && maxdist) {
                if (mindist == maxdist)
                    mprintf(INFO, "{WILDCARD_ANY_STRING(LENGTH==%u)}", mindist);
                else
                    mprintf(INFO, "{WILDCARD_ANY_STRING(LENGTH>=%u&&<=%u)}", mindist, maxdist);
            } else if (mindist)
                mprintf(INFO, "{WILDCARD_ANY_STRING(LENGTH>=%u)}", mindist);
            else if (maxdist)
                mprintf(INFO, "{WILDCARD_ANY_STRING(LENGTH<=%u)}", maxdist);

            if (!(decoded = decodehexspecial(start, &dlen))) {
                mprintf(ERROR, "Decoding failed\n");
                free(hexcpy);
                return -1;
            }
            ...
            if (asterisk)
                mprintf(INFO, "{WILDCARD_ANY_STRING}");
        ...
        for (i = 1; i <= parts; i++) {
            if ((pt = cli_strtok(hexsig, i - 1, "*")) == NULL) {
                mprintf(ERROR, "Can't extract part %u of partial signature\n", i);
                return -1;
            }
...
static int decodecdb(char **tokens)
{
    ...
    mprintf(INFO, "VIRUS NAME: %s\n", tokens[0]);
    mprintf(INFO, "CONTAINER TYPE: %s\n", (strcmp(tokens[1], "*") ? tokens[1] : "ANY"));
    mprintf(INFO, "CONTAINER SIZE: ");
    if (!cli_isnumber(tokens[2])) {
        if (!strcmp(tokens[2], "*")) {
            ...
            if (sz != 2 || !cli_isnumber(range[0]) || !cli_isnumber(range[1])) {
                mprintf(ERROR, "decodesig: Invalid container size range\n");
                return -1;
            }
            mprintf(INFO, "WITHIN RANGE %s to %s\n", range[0], range[1]);

        } else {
            mprintf(ERROR, "decodesig: Invalid container size\n");
            return -1;
        }
    } else {
        mprintf(INFO, "%s\n", tokens[2]);
    }
    mprintf(INFO, "FILENAME REGEX: %s\n", tokens[3]);
    mprintf(INFO, "COMPRESSED FILESIZE: ");
    if (!cli_isnumber(tokens[4])) {
        if (!strcmp(tokens[4], "*")) {
            ...
            mprintf(ERROR, "decodesig: Invalid compressed filesize\n");
            return -1;
        }
    } else {
        mprintf(INFO, "%s\n", tokens[4]);
    }
    mprintf(INFO, "UNCOMPRESSED FILESIZE: ");
    if (!cli_isnumber(tokens[5])) {
        if (!strcmp(tokens[5], "*")) {
            ...
            mprintf(ERROR, "decodesig: Invalid uncompressed filesize\n");
            return -1;
        }
    } else {
        mprintf(INFO, "%s\n", tokens[5]);
    ...
    mprintf(INFO, "ENCRYPTION: ");
    if (!cli_isnumber(tokens[6])) {
        if (!strcmp(tokens[6], "*")) {
            mprintf(INFO, "IGNORED\n");
        } else {
            mprintf(ERROR, "decodesig: Invalid encryption flag\n");
            return -1;
        }
    } else {
    ...
    if (!cli_isnumber(tokens[7])) {
        if (!strcmp(tokens[7], "*")) {
            ...
            mprintf(ERROR, "decodesig: Invalid file position\n");
            return -1;
        }
    } else {
        mprintf(INFO, "%s\n", tokens[7]);
    ...
    if (!strcmp(tokens[1], "CL_TYPE_ZIP") || !strcmp(tokens[1], "CL_TYPE_RAR")) {
        if (!strcmp(tokens[8], "*")) {
            mprintf(INFO, "CRC SUM: ANY\n");
        } else {

            errno = 0;
            sz    = (int)strtol(tokens[8], NULL, 16);
            if (!sz && errno) {
                mprintf(ERROR, "decodesig: Invalid cyclic redundancy check sum\n");
                return -1;
            } else {
                mprintf(INFO, "CRC SUM: %d\n", sz);
            }
        }
    }
...
{
    mprintf(INFO, "FILE TYPE NAME: %s\n", tokens[3]);
    mprintf(INFO, "FILE SIGNATURE TYPE: %s\n", tokens[0]);
    mprintf(INFO, "FILE MAGIC OFFSET: %s\n", tokens[1]);
    mprintf(INFO, "FILE MAGIC HEX: %s\n", tokens[2]);
    mprintf(INFO, "FILE MAGIC DECODED:\n");
    decodehex(tokens[2]);
    mprintf(INFO, "FILE TYPE REQUIRED: %s\n", tokens[4]);
    mprintf(INFO, "FILE TYPE DETECTED: %s\n", tokens[5]);
    if (tokens_count == 7)
        mprintf(INFO, "FTM FLEVEL: >=%s\n", tokens[6]);
    else if (tokens_count == 8)
        mprintf(INFO, "FTM FLEVEL: %s..%s\n", tokens[6], tokens[7]);
    return 0;
}
...
static int decodesig(char *sig, int fd)
{
    ...
    if (*sig == '[') {
        if (!(pt = strchr(sig, ']'))) {
            mprintf(ERROR, "decodesig: Invalid input\n");
            return -1;
        }
        sig = &pt[2];
    }
    ...
    if (strchr(sig, ';')) { /* lsig */
        ...
        if (tokens_count < 4) {
            mprintf(ERROR, "decodesig: Invalid or not supported signature format\n");
            return -1;
        }
        mprintf(INFO, "VIRUS NAME: %s\n", tokens[0]);
        if (strlen(tokens[0]) && strstr(tokens[0], ".{") && tokens[0][strlen(tokens[0]) - 1] == '}')
            bc = 1;
        mprintf(INFO, "TDB: %s\n", tokens[1]);
        mprintf(INFO, "LOGICAL EXPRESSION: %s\n", tokens[2]);
        subsigs = cli_ac_chklsig(tokens[2], tokens[2] + strlen(tokens[2]), NULL, NULL, NULL, 1);
        if (subsigs == -1) {
            mprintf(ERROR, "decodesig: Broken logical expression\n");
            return -1;
        }
        subsigs++;
        if (subsigs > 64) {
            mprintf(ERROR, "decodesig: Too many subsignatures\n");
            return -1;
        }
        if (!bc && subsigs != tokens_count - 3) {
            mprintf(ERROR, "decodesig: The number of subsignatures (==%u) doesn't match the IDs in the logical expression (==%u)\n", tokens_count - 3, subsigs);
            return -1;
        }
        for (i = 0; i < tokens_count - 3; i++) {
            if (i >= subsigs)
                mprintf(INFO, " * BYTECODE SUBSIG\n");
            else
                mprintf(INFO, " * SUBSIG ID %d\n", i);

            subtokens_count = cli_ldbtokenize(tokens[3 + i], ':', 4, (const char **)subtokens, 0);
            if (!subtokens_count) {
                mprintf(ERROR, "decodesig: Invalid or not supported subsignature format\n");
                return -1;
            }
            if ((subtokens_count % 2) == 0)
                mprintf(INFO, " +-> OFFSET: %s\n", subtokens[0]);
            else
                mprintf(INFO, " +-> OFFSET: ANY\n");
            ...
            if (subtokens_count == 3) {
                ...
                mprintf(INFO, " +-> SIGMOD: NONE\n");
            }

            subhex = (subtokens_count % 2) ? subtokens[0] : subtokens[1];
            if (fd == -1) {
                mprintf(INFO, " +-> DECODED SUBSIGNATURE:\n");
                decodehex(subhex);
            } else {
                mprintf(INFO, " +-> ");
        ...
        if (tokens_count < 4 || tokens_count > 6) {
            mprintf(ERROR, "decodesig: Invalid or not supported signature format\n");
            mprintf(INFO, "TOKENS COUNT: %u\n", tokens_count);
            return -1;
        }
        mprintf(INFO, "VIRUS NAME: %s\n", tokens[0]);
        if (tokens_count == 5)
            mprintf(INFO, "FUNCTIONALITY LEVEL: >=%s\n", tokens[4]);
        else if (tokens_count == 6)
            mprintf(INFO, "FUNCTIONALITY LEVEL: %s..%s\n", tokens[4], tokens[5]);

        if (!cli_isnumber(tokens[1])) {
            mprintf(ERROR, "decodesig: Invalid target type\n");
            return -1;
        }
        mprintf(INFO, "TARGET TYPE: ");
        switch (atoi(tokens[1])) {
            case 0:
                mprintf(INFO, "ANY FILE\n");
                break;
            case 1:
                mprintf(INFO, "PE\n");
                break;
            ...
            case 4:
                ...
                break;
            case 5:
                mprintf(INFO, "GRAPHICS\n");
                break;
            case 6:
                mprintf(INFO, "ELF\n");
                break;
            case 7:
                mprintf(INFO, "NORMALIZED ASCII TEXT\n");
                break;
            case 8:
                mprintf(INFO, "DISASM DATA\n");
                break;
            case 9:
                mprintf(INFO, "MACHO\n");
                break;
        ...
        mprintf(INFO, "OFFSET: %s\n", tokens[2]);
        if (fd == -1) {
            mprintf(INFO, "DECODED SIGNATURE:\n");
            decodehex(tokens[3]);
        } else {
        ...
        *pt++ = 0;
        ...
        mprintf(INFO, "decodesig: Not supported signature format\n");
        return -1;
    }
...
static int testsigs(const struct optstruct *opts)
{
    ...
    if (!opts->filename) {
        mprintf(ERROR, "--test-sigs requires two arguments\n");
        return -1;
    }

    sigs = fopen(optget(opts, "test-sigs")->strarg, "rb");
    if (!sigs) {
        mprintf(ERROR, "testsigs: Can't open file %s\n", optget(opts, "test-sigs")->strarg);
        return -1;
    }
...
static int diffdirs(const char *old, const char *new, const char *patch)
{
    ...
    if (!getcwd(cwd, sizeof(cwd))) {
        mprintf(ERROR, "diffdirs: getcwd() failed\n");
        return -1;
    }

    if (!(diff = fopen(patch, "wb"))) {
        mprintf(ERROR, "diffdirs: Can't open %s for writing\n", patch);
        return -1;
    }

    if (chdir(new) == -1) {
        mprintf(ERROR, "diffdirs: Can't chdir to %s\n", new);
        fclose(diff);
        return -1;
    }

    if ((dd = opendir(new)) == NULL) {
        mprintf(ERROR, "diffdirs: Can't open directory %s\n", new);
        fclose(diff);
        return -1;
    }
    ...
    while ((dent = readdir(dd))) {
        if (dent->d_ino) {
            ...
            if (compare(path, dent->d_name, diff) == -1) {
                if (chdir(cwd) == -1)
                    mprintf(WARNING, "diffdirs: Can't chdir to %s\n", cwd);
                fclose(diff);
                unlink(patch);
                closedir(dd);
                return -1;
    ...
    fclose(diff);
    mprintf(INFO, "Generated diff file %s\n", patch);
    if (chdir(cwd) == -1)
        mprintf(WARNING, "diffdirs: Can't chdir to %s\n", cwd);
...
static int makediff(const struct optstruct *opts)
{
    ...
    if (!opts->filename) {
        mprintf(ERROR, "makediff: --diff requires two arguments\n");
        return -1;
    }

    if (!(cvd = cl_cvdhead(opts->filename[0]))) {
        mprintf(ERROR, "makediff: Can't read CVD header from %s\n", opts->filename[0]);
        return -1;
    }
    newver = cvd->version;
    free(cvd);
    ...
    if (oldver + 1 != newver) {
        mprintf(ERROR, "makediff: The old CVD must be %u\n", newver - 1);
        return -1;
    }

    odir = cli_gentemp(NULL);
    if (!odir) {
        mprintf(ERROR, "makediff: Can't generate temporary name for odir\n");
        return -1;
    }

    if (mkdir(odir, 0700) == -1) {
        mprintf(ERROR, "makediff: Can't create directory %s\n", odir);
        free(odir);
        return -1;
    }

    if (cli_cvdunpack(optget(opts, "diff")->strarg, odir) == -1) {
        mprintf(ERROR, "makediff: Can't unpack CVD file %s\n", optget(opts, "diff")->strarg);
        cli_rmdirs(odir);
        free(odir);
        return -1;
    }
    ...
    if (!ndir) {
        mprintf(ERROR, "makediff: Can't generate temporary name for ndir\n");
        cli_rmdirs(odir);
        free(odir);
        return -1;
    }
...
static int dumpcerts(const struct optstruct *opts)
{
    ...
    if (!filename) {
        mprintf(ERROR, "dumpcerts: No filename!\n");
        goto done;
    }
    ...
    if (fd < 0) {
        mprintf(ERROR, "dumpcerts: Can't open file %s!\n", filename);
        goto done;
    }
    ...
    new_map = fmap(fd, 0, sb.st_size, filename);
    if (NULL == new_map) {
        mprintf(ERROR, "dumpcerts: Can't create fmap for open file\n");
        goto done;
    }

    /* build engine */
    if (!(engine = cl_engine_new())) {
        mprintf(ERROR, "dumpcerts: Can't create new engine\n");
        goto done;
    }
    cl_engine_set_num(engine, CL_ENGINE_AC_ONLY, 1);

    if (cli_initroots(engine, 0) != CL_SUCCESS) {
        mprintf(ERROR, "dumpcerts: cli_initroots() failed\n");
        goto done;
    }

    if (cli_parse_add(engine->root[0], "test", "deadbeef", 0, 0, 0, "*", 0, NULL, 0) != CL_SUCCESS) {
        mprintf(ERROR, "dumpcerts: Can't parse signature\n");
        goto done;
    }

    if (cl_engine_compile(engine) != CL_SUCCESS) {
        mprintf(ERROR, "dumpcerts: Can't compile engine\n");
        goto done;
    }
    ...
    switch (ret) {
        ...
        case CL_EVERIFY:
            ...
            break;
        case CL_BREAK:
            mprintf(DEBUG, "dumpcerts: No Authenticode signature detected\n");
            break;
        case CL_EFORMAT:
            mprintf(ERROR, "dumpcerts: An error occurred when parsing the file\n");
            break;
        default:
            mprintf(ERROR, "dumpcerts: Other error %d inside cli_check_auth_header.\n", ret);
            break;
    }
...
{
    mprintf(INFO, "\n");
    mprintf(INFO, "                      Clam AntiVirus: Signature Tool %s\n", get_version());
    mprintf(INFO, "           By The ClamAV Team: https://www.clamav.net/about.html#credits\n");
    mprintf(INFO, "           (C) 2022 Cisco Systems, Inc.\n");
    mprintf(INFO, "\n");
    mprintf(INFO, "    sigtool [options]\n");
    mprintf(INFO, "\n");
    mprintf(INFO, "    --help                 -h              Show this help\n");
    mprintf(INFO, "    --version              -V              Print version number and exit\n");
    mprintf(INFO, "    --quiet                                Be quiet, output only error messages\n");
    mprintf(INFO, "    --debug                                Enable debug messages\n");
    mprintf(INFO, "    --stdout                               Write to stdout instead of stderr. Does not affect 'debug' messages.\n");
    mprintf(INFO, "    --hex-dump                             Convert data from stdin to a hex\n");
    mprintf(INFO, "                                           string and print it on stdout\n");
    mprintf(INFO, "    --md5 [FILES]                          Generate MD5 checksum from stdin\n");
    mprintf(INFO, "                                           or MD5 sigs for FILES\n");
    mprintf(INFO, "    --sha1 [FILES]                         Generate SHA1 checksum from stdin\n");
    mprintf(INFO, "                                           or SHA1 sigs for FILES\n");
    mprintf(INFO, "    --sha256 [FILES]                       Generate SHA256 checksum from stdin\n");
    mprintf(INFO, "                                           or SHA256 sigs for FILES\n");
    mprintf(INFO, "    --mdb [FILES]                          Generate .mdb (section hash) sigs\n");
    mprintf(INFO, "    --imp [FILES]                          Generate .imp (import table hash) sigs\n");
    mprintf(INFO, "    --html-normalise=FILE                  Create normalised parts of HTML file\n");
    mprintf(INFO, "    --ascii-normalise=FILE                 Create normalised text file from ascii source\n");
    mprintf(INFO, "    --utf16-decode=FILE                    Decode UTF16 encoded files\n");
    mprintf(INFO, "    --info=FILE            -i FILE         Print database information\n");
    mprintf(INFO, "    --build=NAME [cvd] -b NAME             Build a CVD file\n");
    mprintf(INFO, "    --max-bad-sigs=NUMBER                  Maximum number of mismatched signatures\n");
    mprintf(INFO, "                                           When building a CVD. Default: 3000\n");
    mprintf(INFO, "    --flevel=FLEVEL                        Specify a custom flevel.\n");
    mprintf(INFO, "                                           Default: %u\n", cl_retflevel());
    mprintf(INFO, "    --cvd-version=NUMBER                   Specify the version number to use for\n");
    mprintf(INFO, "                                           the build. Default is to use the value+1\n");
    mprintf(INFO, "                                           from the current CVD in --datadir.\n");
    mprintf(INFO, "                                           If no datafile is found the default\n");
    mprintf(INFO, "                                           behaviour is to prompt for a version\n");
    mprintf(INFO, "                                           number, this switch will prevent the\n");
    mprintf(INFO, "                                           prompt.  NOTE: If a CVD is found in the\n");
    mprintf(INFO, "                                           --datadir its version+1 is used and\n");
    mprintf(INFO, "                                           this value is ignored.\n");
    mprintf(INFO, "    --no-cdiff                             Don't generate .cdiff file\n");
    mprintf(INFO, "    --unsigned                             Create unsigned database file (.cud)\n");
    mprintf(INFO, "    --hybrid                               Create a hybrid (standard and bytecode) database file\n");
    mprintf(INFO, "    --print-certs=FILE                     Print Authenticode details from a PE\n");
    mprintf(INFO, "    --server=ADDR                          ClamAV Signing Service address\n");
    mprintf(INFO, "    --datadir=DIR                          Use DIR as default database directory\n");
    mprintf(INFO, "    --unpack=FILE          -u FILE         Unpack a CVD/CLD file\n");
    mprintf(INFO, "    --unpack-current=SHORTNAME             Unpack local CVD/CLD into cwd\n");
    mprintf(INFO, "    --list-sigs[=FILE]     -l[FILE]        List signature names\n");
    mprintf(INFO, "    --find-sigs=REGEX      -fREGEX         Find signatures matching REGEX\n");
    mprintf(INFO, "    --decode-sigs                          Decode signatures from stdin\n");
    mprintf(INFO, "    --test-sigs=DATABASE TARGET_FILE       Test signatures from DATABASE against \n");
    mprintf(INFO, "                                           TARGET_FILE\n");
    mprintf(INFO, "    --vba=FILE                             Extract VBA/Word6 macro code\n");
    mprintf(INFO, "    --vba-hex=FILE                         Extract Word6 macro code with hex values\n");
    mprintf(INFO, "    --diff=OLD NEW         -d OLD NEW      Create diff for OLD and NEW CVDs\n");
    mprintf(INFO, "    --compare=OLD NEW      -c OLD NEW      Show diff between OLD and NEW files in\n");
    mprintf(INFO, "                                           cdiff format\n");
    mprintf(INFO, "    --run-cdiff=FILE       -r FILE         Execute update script FILE in cwd\n");
    mprintf(INFO, "    --verify-cdiff=DIFF CVD/CLD            Verify DIFF against CVD/CLD\n");
    mprintf(INFO, "\n");
...
int main(int argc, char **argv)
{
    ...
    if ((ret = cl_init(CL_INIT_DEFAULT)) != CL_SUCCESS) {
        mprintf(ERROR, "Can't initialize libclamav: %s\n", cl_strerror(ret));
        return -1;
    }
    ret = 1;
    ...
    if (optget(opts, "hex-dump")->enabled)
        ...
        if (!opts->filename) {
            mprintf(ERROR, "--verify-cdiff requires two arguments\n");
            ret = -1;
        } else {
            if (CLAMSTAT(opts->filename[0], &sb) == -1) {
                mprintf(INFO, "--verify-cdiff: Can't get status of %s\n", opts->filename[0]);
                ret = -1;
            } else {
                if (S_ISDIR(sb.st_mode))
