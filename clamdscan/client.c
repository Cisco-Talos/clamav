...
static int isremote(const struct optstruct *opts)
{
    ...
    while (opt) {
        ...
        if ((res = getaddrinfo(ipaddr, port, &hints, &info))) {
            logg(ERROR, "Can't lookup clamd hostname: %s\n", gai_strerror(res));
            opt = opt->nextarg;
            continue;
        }
        ...
        for (p = info; p != NULL; p = p->ai_next) {
            if ((s = socket(p->ai_family, p->ai_socktype, p->ai_protocol)) < 0) {
                logg(INFO, "isremote: socket() returning: %s.\n", strerror(errno));
                continue;
            }
...
int16_t ping_clamd(const struct optstruct *opts)
{
    ...
    if (NULL != (opt = optget(opts, "ping"))) {
        if (NULL != opt->strarg) {
            ...
            if (attempt_str + strlen(attempt_str) > errchk) {
                logg(WARNING, "attmept_str would go past end of buffer\n");
                ret = -1;
                goto done;
            }
        } else {
    ...
    do {
        ...
        if (i + 1 < attempts) {
            if (optget(opts, "wait")->enabled) {
                if (interval == 1)
                    logg(DEBUG, "Could not connect, will try again in %lu second\n", interval);
                else
                    logg(DEBUG, "Could not connect, will try again in %lu seconds\n", interval);
            } else {
                if (interval == 1)
                    logg(INFO, "Could not connect, will PING again in %lu second\n", interval);
                else
                    logg(INFO, "Could not connect, will PING again in %lu seconds\n", interval);
            }
            sleep(interval);
        }
        i++;
...
static char *makeabs(const char *basepath)
{
    ...
    if (!(ret = malloc(PATH_MAX + 1))) {
        logg(WARNING, "Can't make room for fullpath.\n");
        return NULL;
    }
    if (!cli_is_abspath(basepath)) {
        if (!getcwd(ret, PATH_MAX)) {
            logg(WARNING, "Can't get absolute pathname of current working directory.\n");
            free(ret);
            return NULL;
        }
#ifdef _WIN32
...
static int client_scan(const char *file, int scantype, int *infected, int *err, int maxlevel, int session, int flags)
{
    ...
    if (CL_SUCCESS != cli_realpath(fullpath, &real_path)) {
        logg(DEBUG, "client_scan: Failed to determine real filename of %s.\n", fullpath);
    } else {
        free(fullpath);
        fullpath = real_path;
    }
...
int get_clamd_version(const struct optstruct *opts)
{
    ...
    while ((len = recvln(&rcv, &buff, NULL))) {
        if (len == -1) {
            logg(ERROR, "Error occurred while receiving version information.\n");
            break;
        }
        printf("%s\n", buff);
    }
...
int reload_clamd_database(const struct optstruct *opts)
{
    ...
    if (!(len = recvln(&rcv, &buff, NULL)) || len < 10 || memcmp(buff, "RELOADING", 9)) {
        logg(ERROR, "Clamd did not reload the database\n");
        closesocket(sockd);
        return 2;
    }
    closesocket(sockd);
...
int client(const struct optstruct *opts, int *infected, int *err)
{
    ...
    *infected = 0;
    ...
    if (scandash) {
        ...
        STATBUF sb;
        if (FSTAT(0, &sb) < 0) {
            logg(INFO, "client.c: fstat failed for file name \"%s\", with %s\n.",
        ...
        while ((fname = filelist(opts, NULL))) {
            if (!strcmp(fname, "-")) {
                logg(ERROR, "Scanning from standard input requires \"-\" to be the only file argument\n");
                continue;
            }
            errors += client_scan(fname, scantype, infected, err, maxrec, session, flags);
            /* this may be too strict
	    if(errors >= 10) {
		logg(ERROR, "Too many errors\n");
		break;
	    }
	    */
        }
