...
int upx_inflatelzma(const char *src, uint32_t ssize, char *dst, uint32_t *dsize, uint32_t upx0, uint32_t upx1, uint32_t ep, uint32_t properties)
{
    ...
    /* ssize-2u below: guard against underflow on adversarial input   */
    if (ssize < 3u)
        return -1;
    ...
    *fake_lzmahdr = lc + 9 * (5 * pb + lp);
    ...
    /* avail_in must be ssize-2: next_in is already 2 bytes past src,
     * so passing ssize would allow the decoder to read 2 bytes past
     * the end of the validated compressed section.                  */
    l.avail_in  = ssize - 2u;
    l.avail_out = *dsize;
                         ...
                         uint32_t properties, uint32_t *magic)
{
     ...
     * read 2 bytes past the end of the validated compressed section.  */
    l.avail_in  = ssize - 2u;
    l.avail_out = *dsize;
    l.next_in   = (unsigned char *)src + 2;
    l.next_out  = (unsigned char *)dst;
                    ...
                    const char *epbuff, size_t epbuff_len)
{
     ...
     * [FROM FILE]: all values read from epbuff are bounds-checked.   */
    {
        ...
        {
            int rc = upx_inflatelzma(src, ssize, dst, &strictdsize,
                                     upx0_rva, upx1_rva, ep_rva,
                                     properties);
            /* Propagate actual output size back to caller -- mirrors the
             * same fix applied to upx_unpack_pe64.  Without this write-back
             * the caller uses the stale original dsize, scanning trailing
             * zeros beyond the actual rebuilt PE content.              */
            if (rc >= 0)
                *dsize = strictdsize;
            return rc;
        }
                    ...
                    uint32_t *magic, int stub_type)
{
    ...
    if (stub_type == UPX_STUB_LZMA) {
        ...
        {
            int rc = upx_inflatelzma_pe64(src, ssize, dst, &strictdsize,
                                          upx0_rva, upx1_rva, ep_rva,
                                          properties, magic);
            /* Propagate the actual output size back to the caller.
             * upx_inflatelzma_pe64 updates strictdsize to the number of
             * bytes written; the caller (pe.c / clam_upx.c) needs this
             * value to know how much of dst[] to scan.  Without this
             * write-back the caller uses the stale original dsize,
             * risking scanning padded zeros or truncated output.     */
            if (rc >= 0)
                *dsize = strictdsize;
            return rc;
        }
