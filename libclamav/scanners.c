...
static cl_error_t cli_scanhtml(cli_ctx *ctx)
{
    ...
    if (fd >= 0) {
        if (CL_VIRUS == (ret = cli_scan_desc(fd, ctx, CL_TYPE_HTML, false, NULL, AC_SCAN_VIR,
    ...
    if (ret == CL_CLEAN || (ret == CL_VIRUS && SCAN_ALLMATCHES)) {
        ...
        if (fd >= 0) {
            ...
            if (ret == CL_CLEAN || (ret == CL_VIRUS && SCAN_ALLMATCHES)) {
                if (CL_VIRUS == (ret = cli_scan_desc(fd, ctx, CL_TYPE_TEXT_ASCII, false, NULL, AC_SCAN_VIR,
                                                     NULL, NULL, LAYER_ATTRIBUTES_NORMALIZED))) {
                    viruses_found++;
                }
            }
