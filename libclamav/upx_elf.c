 ...
 *
 * Method numbers are stable across all known UPX versions; the method
 * byte values 2/5/8/14 have not changed since UPX 1.x.  UPX 1.x only
 * shipped the NRV family; LZMA (14) was introduced in UPX 2.x.      */
...
#define V1_MIN_FILE_SIZE  189u
   ...
   *
   * Method numbering is stable across all known UPX versions: the values
   * 2/5/8/14 for NRV2B/NRV2D/NRV2E/LZMA have not changed since UPX 1.x.
   * UPX 1.x packed ELF used only NRV variants; LZMA (14) was added in UPX 2.x.
   *
 ...
 *
 * UPX LZMA 2-byte header format (verified by inspection of packed binaries):
	...
	uint32_t properties)
{
	 ...
	 * avail_in must be ssize-2, not ssize: next_in is already advanced
	 * 2 bytes past src, so passing ssize would allow the decoder to
	 * read 2 bytes past the end of the validated compressed block.   */
	l.avail_in = ssize - 2u;
	l.avail_out = *dsize;
 ...
 *     UPX 2.x and UPX 3.09 zero the start l_info when packing ELF64
 *     binaries, as confirmed by inspection of packed ELF64 samples from
 *     UPX versions 2.x through 3.09.  The start l_info at phdrs_end is
 *     all zeros; the real l_info with UPX! magic is only at the END of
 *     the file (before the pack header).
 *
	...
	size_t* li_off_out, int* is64_out)
{
	 ...
	 *
	 * Overflow analysis:
	 *   e_phnum   <= MAX_PHNUM (16)
	 *   phdr_size <= ELF64_PHDR_SIZE (56)
	 *   product   <= 16 * 56 = 896  -- cannot overflow uint64_t
	 *
	 *   e_phoff is a uint64_t read directly from the file.  A crafted
	 *   ELF64 can set e_phoff near UINT64_MAX so that
	 *   e_phoff + product wraps around and passes the > fsz check.
	 *   Guard with an explicit subtraction-based check that avoids
	 *   the addition entirely:
	 *     e_phoff + table_size <= fsz
	 *   <=>  table_size <= fsz - e_phoff   (only valid when e_phoff <= fsz)
	 *
	 *   We also guard the (size_t) cast: on 32-bit hosts size_t is 32
	 *   bits, so a uint64_t result > SIZE_MAX would silently truncate.
	 *   The fsz <= SIZE_MAX invariant (it came from ftell/fmap) means
	 *   any value that passes the <= fsz check is also <= SIZE_MAX.   */
	{
		uint64_t table_size = (uint64_t)e_phnum * (uint64_t)phdr_size;
		if (e_phoff < (uint64_t)hdr_min)
			return 0;
		/* e_phoff > fsz: phdr table starts past EOF                 */
		if (e_phoff > (uint64_t)fsz)
			return 0;
		/* table_size > fsz - e_phoff: phdr table extends past EOF   */
		if (table_size > (uint64_t)fsz - e_phoff)
			return 0;
	}

	phdrs_end = (size_t)e_phoff + (size_t)e_phnum * phdr_size;
	...
	/* -- l_info location: three probes in order, stop at first hit ----
	 *
	 * We need to find the 12-byte l_info struct between the phdr table
	 * and the compressed payload.  Its exact position varies by UPX
	 * version and binary type, so we try three probes.
	 *
	 * li_off tracks the resolved l_info offset.  phdrs_end retains its
	 * original meaning (end of phdr table) throughout; we only copy the
	 * final li_off back to phdrs_end at the end of this block so that
	 * downstream code (written before li_off was introduced) keeps
	 * working without change.                                         */
	{
		size_t li_off = phdrs_end;

		/* SECURITY: minimum space needed for any l_info probe            */
		if (li_off + L_INFO_SIZE + 4 > fsz)
			return 0;

		/* -- Probe 1: standard position (phdrs_end) ----------------------
		 * l_info: checksum(4) + magic(4) + lsize(2) + ver(1) + fmt(1)
		 * Magic sits at offset +4 from the l_info start.                */
		li_magic = rd32(data + li_off + 4);   /* [FROM FILE] */

		/* -- Probe 2: extended l_info, magic at phdrs_end+8 (ELF64 only)
		 * Some UPX 2.x ELF64 binaries use a 16-byte l_info where the
		 * checksum occupies 8 bytes instead of 4, shifting magic to +8.
		 * The logical l_info start is phdrs_end+4 so that
		 * li_off + L_INFO_SIZE correctly reaches p_info.                */
		...
		/* -- Probe 3: PT_NOTE skip (PIE / ET_DYN binaries) ---------------
		 * UPX-packed PIE ELF32 binaries emit a PT_NOTE segment immediately
		 * after the phdr table, pushing l_info past phdrs_end.  Scan for
		 * an adjacent PT_NOTE and probe for UPX magic right after it.   */
		if (li_magic != UPX_MAGIC_V2 && li_magic != UPX_MAGIC_V1) {
			uint16_t pi;
		...
		/* -- end probe sequence; li_magic and li_off are final ---------- */

		phdrs_end = li_off;   /* downstream uses phdrs_end as l_info offset */
	} /* end l_info probe block */
	...
	if (li_magic == UPX_MAGIC_V2) {
		...
		if (load0_filesz < load0_memsz &&
			...
			*is64_out = is64;
			...
			load1_filesz == 0 &&
			load1_offset == 0) {
			/* UPX 3.95 --brute ET_EXEC pattern:
			 *   PT_LOAD[0]: filesz==memsz (whole packed file, no BSS)
			 *   PT_LOAD[1]: offset==0, filesz==0 (output region)
			 * Observed in real UPX 3.95 --brute ELF64 ET_EXEC binaries.
			 * Distinct from UPX 2.x pattern where load1_offset > 0.  */
			if (load0_filesz > (uint64_t)fsz) {
				cli_dbgmsg("elf_upx: UPX3-brute load0_filesz 0x%llx > fsz 0x%zx\n",
					(unsigned long long)load0_filesz, fsz);
				return 0;
			}
			*li_off_out = phdrs_end;
			*is64_out = is64;
			return 3;   /* UPX 3.x+ brute */
		...
		*is64_out = is64;
		 ...
		 *
		 * UPX 2.x and 3.09 zero the start l_info when packing ELF64
		 * binaries, as confirmed by inspection of packed ELF64 samples.
		 * The entire 12-byte l_info block at phdrs_end is zero.  The
		 * real l_info with UPX! magic is appended at the end of the
		 * file (the "end l_info").
		 *
		 * Detection strategy:
		 ...
		 * further since both use the same b_info decompressor.       */
		{
			 ...
			 * very last L_INFO_SIZE bytes of the file.               */
			{
				...
				/* Scan downward from search_start to search_end.
				 * s is size_t (unsigned): when s wraps past 0 it becomes
				 * SIZE_MAX.  The guard (s <= search_start) catches that
				 * wrap and exits the loop before the body can run with a
				 * garbage index.                                        */
	...
	uint32_t* out_used)
{
	...
	int      overlap_stop = 0; /* set when we stop due to UPX 2.x overlap blocks */
	int      clean_stop = 0; /* set when we stop at EOF terminator or p_filesize */
	...
	*out_used = 0;
	...
	end_li_off = 0;
	{
		/* Scan forward to find the LAST occurrence of UPX! magic.
		 * The end pack_hdr_a is always the rightmost one in the file;
		 * forward scan with no break gives us that directly.        */
		size_t s;
	...
	while (block_num < MAX_BLOCKS) {
		...
		/* UPX applies a call/jump filter to x86 and x64 code blocks before
		 * compression (ftid=0x49 for x86 E8/E9, ftid=0x46 for x64).  The
		 * decompressor should reverse the filter so signatures match the
		 * original bytes.  Filter unapplication is not yet implemented here;
		 * blocks with chunk_filter_id != 0 are decompressed and scanned in
		 * their filtered (transformed) form.  The bytecode hook fallback in
		 * cli_unpackelf() may produce better results for filtered samples.
		 * Implementing the unfilter is deferred to a follow-on change.     */
		(void)chunk_filter_param;

		/* Explicit block terminator: clean stop, output is complete */
		if (chunk_out_len == 0) {
			clean_stop = 1;
			break;
		}
		 ...
		 * chunk_out_len > p_blocksize means we've walked past the real
		 * blocks into loader code that wasn't packed.  All real content
		 * has already been decompressed; treat this as a clean stop.
		 * Do NOT hard-fail here -- total_out is valid at this point.  */
		...
		case METHOD_NRV2B32:
			 ...
			 * Use the _raw entry points -- ELF output is raw binary,
			 * not a PE.  The normal inflate engines tail-call pefromupx()
			 * which would overwrite the decompressed data with a fake
			 * PE header.  The _raw variants decompress into dst[]
			 * directly with no PE reconstruction.                       */
		 ...
		 * Note: for UPX 4.x ELF, p_filesize equals the original stat
		 * size which may exceed sum(chunk_out_len) because section
		 * headers are not in the block stream.  total_out reaching
		 * p_filesize is a clean-stop condition when it fires.        */
		if (total_out >= p_filesize) {
			clean_stop = 1;
			break;
		}
	...
	/* SECURITY: verify decompression reached a clean stopping point.
	 *
	 * Three legitimate stop conditions (any sets clean_stop or overlap_stop):
	 *   1. chunk_out_len == 0  EOF terminator  (clean_stop)
	 *   2. total_out >= p_filesize             (clean_stop)
	 *   3. UPX 2.x overlap block heuristic     (overlap_stop)
	 *
	 * Any other exit from the block walk (bi_off overflow, bad block
	 * header, inflate failure, safety cap) is a corruption indicator
	 * and must fail so the bytecode hook fallback can be attempted.
	 *
	 * NOTE: for UPX 4.x ELF64, p_filesize is the original stat size
	 * (including section headers not in the block stream), so
	 * total_out < p_filesize is normal and does NOT indicate truncation.
	 * The EOF terminator (chunk_out_len==0) is the reliable stop signal.
	 *
	 * For UPX 2.x ELF32, overlap blocks prevent full output; we accept
	 * partial output in that case because the missing tail is only ELF
	 * section header metadata, not executable code.                    */
	if (!clean_stop && !overlap_stop) {
		cli_dbgmsg("elf_upx: block walk stopped without clean terminator "
			"after %u blocks (total_out=0x%x p_filesize=0x%x) "
			"-- possible corruption\n",
			block_num, total_out, p_filesize);
		return 0;
	}

	cli_dbgmsg("elf_upx: decompressed %u blocks, 0x%x bytes "
		"(p_filesize=0x%x%s)\n",
		block_num, total_out, p_filesize,
		overlap_stop ? ", overlap-block partial" : "");
	...
	uint8_t** out, uint32_t* out_used)
{
	...
	*out_used = 0;
	...
	if (upx_ver == 2 || upx_ver == 3 || upx_ver == 4) {
		...
		/* SECURITY: compare as size_t -- if fsz > 4GB, fsz-8 exceeds
		 * UINT32_MAX and a uint32_t comparison would always be false. */
		if (fsz < 8u || (size_t)lsize > fsz - 8u)
			return -2;
