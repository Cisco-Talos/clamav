     ...
     */
    {
        ...
        if (filebuf) {
            ...
            if (upx_ret == 0) {
                ...
                /* Do NOT return here: fall through to the bytecode hook so it
                 * gets a second pass on the decompressed output.  The bytecode
                 * database may contain signatures that match only after the
                 * unpacked image is re-processed, and partial native output
                 * (e.g. UPX 2.x overlap-block case) may be completed by the
                 * bytecode unpacker.  If the scan already found a virus, ret
                 * carries that result and the bytecode hook will see it too.  */
                if (ret == CL_VIRUS)
                    goto done;
