...
static CFTypeRef _get_cert_ref(keychain_type_t keychain_type)
{
    ...
    if (status != errSecSuccess) {
        mprintf(ERROR, "Failed to open %s keychain: %s (%d)\n",
                kc_info.name,
                kc_info.file_path,
                status);
        goto done;
    ...
    if (status != errSecSuccess) {
        mprintf(ERROR, "Failed to get the status of the %s keychain: %d\n",
                kc_info.name,
                status);
        goto done;
    }
    if (!(keychainStatus & kSecReadPermStatus)) {
        mprintf(ERROR, "The %s keychain is not readable: %" PRIu32 "\n",
                kc_info.name,
                keychainStatus);
        goto done;
    }
    ...
    if (keychain_type == KEYCHAIN_TYPE_SYSTEM_ROOT) {
        ...
        if (keychainStatus & (kSecUnlockStateStatus | kSecWritePermStatus)) {
            mprintf(ERROR, "System Root Certificates Keychain has invalid permissions: %" PRIu32 "\n",
                    keychainStatus);
            /* continue on error */
        }
    }
    ...
    if (search_list == NULL) {
        mprintf(ERROR, "Failed to create %s keychain search list\n",
                kc_info.name);
        goto done;
    }
    ...
    if (query == NULL) {
        mprintf(ERROR, "Failed to create %s keychain query dictionary\n",
                kc_info.name);
        goto done;
    }
    ...
    if (status != errSecSuccess) {
        if (status == errSecItemNotFound) {
            mprintf(DEBUG, "No items found in %s keychain\n",
                    kc_info.name);
        } else {
            mprintf(ERROR, "Unable to copy certificates from %s keychain (%d)\n",
                    kc_info.name,
                    status);
        }
    }
...
cl_error_t cert_store_load(X509 **trusted_certs, size_t trusted_cert_count)
{
    ...
    for (kc_index = 0; kc_index < ARRAY_SIZE(keychains); kc_index++) {
        ...
        if (CFGetTypeID(items) != CFArrayGetTypeID()) {
            mprintf(ERROR, "Expected array of certificates from %s keychain, "
                    "got type %lu\n",
                    kc_info.name,
                    CFGetTypeID(items));
            continue;
        ...
        mprintf(DEBUG, "Found %ld certificates from %s keychain\n",
                kc_data->certs_count,
                kc_info.name);
    ...
    if (total_certificates < 1) {
        mprintf(ERROR, "No certificate found in keychains. Expect at least one "
    ...
    if (store->loaded) {
        mprintf(DEBUG, "Cert store already loaded\n");
        ret = CL_SUCCESS;
        goto done;
    }
    ...
    if (store->system_certs.certificates == NULL) {
        mprintf(ERROR, "Failed to reserve memory for system cert list\n");
        goto done;
    }
        ...
        for (i = 0; i < kc_data->certs_count; i++) {
            ...
            if (CFGetTypeID(value) == SecCertificateGetTypeID()) {
                ...
                if (cert_data) {
                    ...
                    if (x509) {
                        mprintf(DEBUG, "Found %s trusted certificate %s\n",
                                kc_info.name,
                                (name ? name : "<no name>"));

                        store->system_certs.certificates[store->system_certs.count++] = x509;
                    } else {
                        mprintf(ERROR, "Failed conversion of DER format to X.509\n");
                    }
#if OPENSSL_VERSION_NUMBER >= 0x10100000L
                    if (NULL != name) {
                        free(name);
