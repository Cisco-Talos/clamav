...
cl_error_t cert_store_load(X509 **trusted_certs, size_t trusted_cert_count)
{
    ...
    if (NULL == hStore) {
        mprintf(ERROR, "Failed to open system certificate store.\n");
        goto done;
    }
    ...
    if (store->loaded) {
        mprintf(INFO, "Cert store already loaded\n");
        ret = CL_SUCCESS;
        goto done;
    }
    ...
    while (NULL != (pWinCertContext = CertEnumCertificatesInStore(hStore, pWinCertContext))) {
        ...
        if (NULL == x509) {
            mprintf(ERROR, "Failed to convert system certificate to x509.\n");
            continue;
        }
        ...
        if (mprintf_verbose) {
            ...
            if (NULL == issuer) {
                mprintf(ERROR, "Failed to allocate memory for certificate name.\n");
                ret = CURLE_OUT_OF_MEMORY;
                goto done;
            }

            if (0 == CertGetNameStringA(pWinCertContext, CERT_NAME_FRIENDLY_DISPLAY_TYPE, CERT_NAME_ISSUER_FLAG, NULL, issuer, issuerLen)) {
                mprintf(ERROR, "Failed to get friendly display name for certificate.\n");
            } else {
                mprintf(INFO, "Certificate loaded from Windows certificate store: %s\n", issuer);
            }
    ...
    switch (lastError) {
        case E_INVALIDARG:
            mprintf(ERROR, "The handle in the hCertStore parameter is not the same as that in the certificate context pointed to by pPrevCertContext.\n");
            break;
        case CRYPT_E_NOT_FOUND:
        case ERROR_NO_MORE_FILES:
            if (0 == numCertificatesFound) {
                mprintf(ERROR, "No certificates were found.\n");
            }
            break;
        default:
            mprintf(ERROR, "Unexpected error code from CertEnumCertificatesInStore()\n");
    }
