...
void prelude_logging(const char *filename, const char *virname, const char *virhash, int virsize)
{
    ...
    logg(INFO, "You have to compile with libprelude using ./configure --enable-prelude\n");
}
#else
...
int prelude_initialize_client(const char *analyzer_name)
{
    ...
    if (ret < 0) {
        logg(INFO, "Unable to initialize the prelude library : %s", prelude_strerror(ret));
        return -1;
    }

    ret = prelude_client_new(&prelude_client, analyzer_name);
    if (ret < 0) {
        logg(INFO, "Unable to create a prelude client object : %s", prelude_strerror(ret));
        return -1;
    }

    ret = idmef_analyzer_setup(prelude_client_get_analyzer(prelude_client), analyzer_name);
    if (ret < 0) {
        logg(INFO, "%s", prelude_strerror(ret));
        return -1;
    }

    ret = prelude_client_start(prelude_client);
    if (ret < 0 || !prelude_client) {
        logg(INFO, "Unable to start prelude client : %s", prelude_strerror(ret));
    ...
    if (ret < 0) {
        logg(INFO, "Unable to send asynchronous send and timer : %s", prelude_strerror(ret));
        prelude_client_destroy(prelude_client, PRELUDE_CLIENT_EXIT_STATUS_SUCCESS);
        return -1;
    }
...
void prelude_logging(const char *filename, const char *virname, const char *virhash, int virsize)
{
    ...
    logg(INFO, "le client : %s", prelude_client_get_config_filename(prelude_client));
...
err:
    ...
    logg(INFO, "%s error: %s", prelude_strsource(ret), prelude_strerror(ret));
    return;
}
#endif
