...
- Freshclam, ClamD, ClamScan, and Sigtool: Added an option to enable FIPS-like
  ...
  For `clamscan` and `sigtool` use this commandline option:
  ```
  --fips-limits
  ```
...
- ClamScan: Improved the precision of the bytes-scanned and bytes-read counters.
  ...
  or "B" as appropriate. Previously, it always report "MB".
...
- ClamScan: Add hash & file-type in/out CLI options:
  ...
  We will not be adding this for ClamDScan, as we don't have a mechanism in the
...
- libclamav: Added a class of scan callback functions that can be added with the
  ...
  - `CL_SCAN_CALLBACK_POST_SCAN`: Occurs after pattern matching and after
    running parser modules (i.e. scan complete for this layer).
  ...
  - `CL_VIRUS`: This means you don't trust the file. A new alert will be added.
    For `CL_SCAN_CALLBACK_ALERT`: Means you agree with the alert (no extra alert
    needed).
  ...
  get previous layers, can get the the layer's fmap, and then various attributes
  of the layer and of the fmap. To make this possible, there are new APIs to
...
- Signature matches with names that start with "Weak." will no longer alert.
