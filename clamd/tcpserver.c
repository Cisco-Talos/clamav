...
int tcpserver(int **lsockets, unsigned int *nlsockets, char *ipaddr, const struct optstruct *opts)
{
    ...
    if (num_fd > 2) {
        logg(ERROR, "TCP: Received more than two file descriptors from systemd.\n");
        return -1;
    } else if (num_fd > 0) {
        ...
        for (i = 0; i < num_fd; i += 1) {
            ...
            if (sd_is_socket(sockfd, AF_INET, SOCK_STREAM, 1) == 1) {
                /* correct socket */
                logg(INFO_NF, "TCP: Received AF_INET SOCK_STREAM socket from systemd.\n");
                break;
            } else if (sd_is_socket(sockfd, AF_INET6, SOCK_STREAM, 1) == 1) {
                /* correct socket */
                logg(INFO_NF, "TCP: Received AF_INET6 SOCK_STREAM socket from systemd.\n");
                break;
            } else {
                /* wrong socket */
                sockfd = -2;
        ...
        if (sockfd == -2) {
            logg(INFO_NF, "TCP: No tcp AF_INET/AF_INET6 SOCK_STREAM socket received from systemd.\n");
            return -2;
        }
    ...
    if ((res = getaddrinfo(ipaddr, port, &hints, &info))) {
        logg(ERROR, "TCP: getaddrinfo failed: %s\n", gai_strerror(res));
        return -1;
    }
    ...
    for (p = info; p != NULL; p = p->ai_next, i++) {
        ...
        if ((sockfd = socket(p->ai_family, p->ai_socktype, p->ai_protocol)) == -1) {
            estr = strerror(errno);
            logg(ERROR, "TCP: socket() error: %s\n", estr);
            continue;
        }

        if (setsockopt(sockfd, SOL_SOCKET, SO_REUSEADDR, (void *)&yes, sizeof(yes)) == -1) {
            logg(ERROR, "TCP: setsocktopt(SO_REUSEADDR) error: %s\n", strerror(errno));
        }
        ...
        if (p->ai_family == AF_INET6 &&
            ...
            estr = strerror(errno);
            logg(ERROR, "TCP: setsocktopt(IPV6_V6ONLY) error: %s\n", estr);
        }
#endif /* IPV6_V6ONLY */
        ...
        if ((res = getnameinfo(p->ai_addr, p->ai_addrlen, host, sizeof(host),
            ...
            logg(ERROR, "TCP: getnameinfo failed: %s\n", gai_strerror(res));
            host[0] = '\0';
            serv[0] = '\0';
        }
#else
        ...
        if (bind(sockfd, p->ai_addr, p->ai_addrlen) == -1) {
            estr = strerror(errno);
            logg(ERROR, "TCP: Cannot bind to [%s]:%s: %s\n", host, serv, estr);
            closesocket(sockfd);

            continue;
        }
        logg(INFO_NF, "TCP: Bound to [%s]:%s\n", host, serv);

        backlog = optget(opts, "MaxConnectionQueueLength")->numarg;
        logg(INFO_NF, "TCP: Setting connection queue length to %d\n", backlog);

        if (listen(sockfd, backlog) == -1) {
            estr = strerror(errno);
            logg(ERROR, "TCP: Cannot listen on [%s]:%s: %s\n", host, serv, estr);
            closesocket(sockfd);
