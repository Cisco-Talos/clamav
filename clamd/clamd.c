...
int main(int argc, char **argv)
{
    ...
    if ((opt = optget(opts, "LogFile"))->enabled) {
        ...
        time(&currtime);
        if (logg(INFO_NF, "+++ Started at %s", cli_ctime(&currtime, timestr, sizeof(timestr)))) {
    ...
    if (foreground == 0) {
        ...
        for (ret = 0; (unsigned int)ret < nlsockets; ret++) {
            if (fcntl(lsockets[ret], F_SETFL, fcntl(lsockets[ret], F_GETFL) | O_NONBLOCK) == -1) {
                logg(ERROR, "fcntl for lsockets[] failed\n");
                close(lsockets[ret]);
                ret = 1;
                break;
            }
    ...
    if ((opt = optget(opts, "PidFile"))->enabled) {
        ...
        if ((fd = fopen(opt->strarg, "w")) == NULL) {
            //logg(ERROR, "Can't save PID in file %s\n", opt->strarg);
    ...
    do { /* logger initialized */
        ...
        if ((ret = cl_init(CL_INIT_DEFAULT))) {
            logg(ERROR, "Can't initialize libclamav: %s\n", cl_strerror(ret));
            ret = 1;
            break;
        }
        ...
        if (optget(opts, "LogSyslog")->enabled) {
            ...
            if ((fac = logg_facility(opt->strarg)) == -1) {
                logg(ERROR, "LogFacility: %s: No such facility.\n", opt->strarg);
                ret = 1;
                break;
            }
        ...
        logg(INFO_NF, "Received %d file descriptor(s) from systemd.\n", num_fd);

        if (!tcpsock && !localsock && num_fd == 0) {
            logg(ERROR, "Please define server type (local and/or TCP).\n");
            ret = 1;
            break;
        }

        logg(INFO_NF, "clamd daemon %s (OS: " TARGET_OS_TYPE ", ARCH: " TARGET_ARCH_TYPE ", CPU: " TARGET_CPU_TYPE ")\n", get_version());

#ifndef _WIN32
        if (user)
            logg(INFO_NF, "Running as user %s (UID %u, GID %u)\n", user->pw_name, user->pw_uid, user->pw_gid);
#endif
            ...
            * Set limit lower than 2G if on 32-bit */
            ...
            if (sizeof(void *) == 4 &&
                ...
                if (setrlimit(RLIMIT_DATA, &rlim) < 0)
                    logg(ERROR, "setrlimit(RLIMIT_DATA) failed: %s\n", strerror(errno));
                else
                    logg(INFO, "Running on 32-bit system, and RLIMIT_DATA > 2GB, lowering to 2GB!\n");
            }
        }
#endif

        if (logg_size)
            logg(INFO_NF, "Log file size limited to %lld bytes.\n", (long long int)logg_size);
        else
            logg(INFO_NF, "Log file size limit disabled.\n");

        min_port = optget(opts, "StreamMinPort")->numarg;
        max_port = optget(opts, "StreamMaxPort")->numarg;
        if (min_port < 1024 || min_port > max_port || max_port > 65535) {
            logg(ERROR, "Invalid StreamMinPort/StreamMaxPort: %d, %d\n", min_port, max_port);
            ret = 1;
            break;
        }

        if (!(engine = cl_engine_new())) {
            logg(ERROR, "Can't initialize antivirus engine\n");
            ret = 1;
            break;
        }
        ...
        logg(INFO_NF, "Reading databases from %s\n", dbdir);
        ...
        if (optget(opts, "DetectPUA")->enabled) {
            ...
            if ((opt = optget(opts, "ExcludePUA"))->enabled) {
                ...
                i = 0;
                logg(INFO_NF, "Excluded PUA categories:");

                while (opt) {
                    if (!(pua_cats = realloc(pua_cats, i + strlen(opt->strarg) + 3))) {
                        logg(ERROR, "Can't allocate memory for pua_cats\n");
                        cl_engine_free(engine);
                        ret = 1;
                        break;
                    }

                    logg(INFO_NF, " %s", opt->strarg);
            ...
            if ((opt = optget(opts, "IncludePUA"))->enabled) {
                if (pua_cats) {
                    logg(ERROR, "ExcludePUA and IncludePUA cannot be used at the same time\n");
                    free(pua_cats);
                    ret = 1;
                    break;
                }
                ...
                i = 0;
                logg(INFO_NF, "Included PUA categories:");
                while (opt) {
                    if (!(pua_cats = realloc(pua_cats, i + strlen(opt->strarg) + 3))) {
                        logg(ERROR, "Can't allocate memory for pua_cats\n");
                        ret = 1;
            ...
            if (pua_cats) {
                if ((ret = cl_engine_set_str(engine, CL_ENGINE_PUA_CATEGORIES, pua_cats))) {
                    logg(ERROR, "cli_engine_set_str(CL_ENGINE_PUA_CATEGORIES) failed: %s\n", cl_strerror(ret));
                    free(pua_cats);
                    ret = 1;
                    break;
                }
            ...
            logg(INFO_NF, "Not loading PUA signatures.\n");
        }

        if (optget(opts, "OfficialDatabaseOnly")->enabled) {
            dboptions |= CL_DB_OFFICIAL_ONLY;
            logg(INFO_NF, "Only loading official signatures.\n");
        }

        /* set the temporary dir */
        if ((opt = optget(opts, "TemporaryDirectory"))->enabled) {
            if ((ret = cl_engine_set_str(engine, CL_ENGINE_TMPDIR, opt->strarg))) {
                logg(ERROR, "cli_engine_set_str(CL_ENGINE_TMPDIR) failed: %s\n", cl_strerror(ret));
                ret = 1;
                break;
            }
        }
        ...
        if (optget(opts, "PhishingSignatures")->enabled)
            ...
            logg(INFO_NF, "Not loading phishing signatures.\n");
        ...
        if (optget(opts, "Bytecode")->enabled) {
            ...
            if ((opt = optget(opts, "BytecodeSecurity"))->enabled) {
                ...
                if (!strcmp(opt->strarg, "TrustSigned")) {
                    ...
                    logg(INFO_NF, "Bytecode: Security mode set to \"TrustSigned\".\n");
                } else if (!strcmp(opt->strarg, "Paranoid")) {
                    s = CL_BYTECODE_TRUST_NOTHING;
                    logg(INFO_NF, "Bytecode: Security mode set to \"Paranoid\".\n");
                } else {
                    logg(ERROR, "Unable to parse bytecode security setting:%s\n",
                         opt->strarg);
                    ret = 1;
                    break;
                }
                ...
                if ((ret = cl_engine_set_num(engine, CL_ENGINE_BYTECODE_SECURITY, s))) {
                    logg(WARNING, "Invalid bytecode security setting %s: %s\n", opt->strarg, cl_strerror(ret));
                    ret = 1;
                    break;
                }
            }
            if ((opt = optget(opts, "BytecodeUnsigned"))->enabled) {
                ...
                logg(INFO_NF, "Bytecode: Enabled support for unsigned bytecode.\n");
            }
        ...
        logg(INFO_NF, "Loaded %u signatures.\n", sigs);
        ...
        if ((opt = optget(opts, "PCREMatchLimit"))->active) {
            if ((ret = cl_engine_set_num(engine, CL_ENGINE_PCRE_MATCH_LIMIT, opt->numarg))) {
                logg(ERROR, "cli_engine_set_num(PCREMatchLimit) failed: %s\n", cl_strerror(ret));
                cl_engine_free(engine);
                return 1;
            }
        }
        ...
        if ((opt = optget(opts, "PCRERecMatchLimit"))->active) {
            if ((ret = cl_engine_set_num(engine, CL_ENGINE_PCRE_RECMATCH_LIMIT, opt->numarg))) {
                logg(ERROR, "cli_engine_set_num(PCRERecMatchLimit) failed: %s\n", cl_strerror(ret));
                cl_engine_free(engine);
                return 1;
            }
        }
        ...
        if ((ret = cl_engine_compile(engine)) != 0) {
            logg(ERROR, "Database initialization error: %s\n", cl_strerror(ret));
            ret = 1;
            break;
        }
        ...
        if (localsock && num_fd == 0) {
            ...
            if (optget(opts, "LocalSocketGroup")->enabled) {
                ...
                if (chown(optget(opts, "LocalSocket")->strarg, -1, sock_gid)) {
                    logg(ERROR, "Failed to change socket ownership to group %s\n", gname);
                    ret = 1;
                    break;
                }
            }
            if (optget(opts, "LocalSocketMode")->enabled) {
                ...
                if (*end) {
                    logg(ERROR, "Invalid LocalSocketMode %s\n", optget(opts, "LocalSocketMode")->strarg);
                    ret = 1;
                    break;
                }
            } else {
            ...
            if (chmod(optget(opts, "LocalSocket")->strarg, sock_mode & 0666)) {
                logg(ERROR, "Cannot set socket permission for %s to %3o\n", optget(opts, "LocalSocket")->strarg, sock_mode & 0666);
                ret = 1;
                break;
            }
        ...
        if (0 == foreground) {
            if (!debug_mode) {
                if (chdir("/") == -1) {
                    logg(WARNING, "Can't change current working directory to root\n");
                }
            }
        ...
        if (nlsockets == 0) {
            logg(ERROR, "Not listening on any interfaces\n");
            ret = 1;
            break;
        }
    ...
    if (num_fd == 0) {
        logg(DEBUG, "Closing the main socket%s.\n", (nlsockets > 1) ? "s" : "");
        ...
        if (nlsockets && localsock) {
            ...
            if (unlink(opt->strarg) == -1)
                logg(ERROR, "Can't unlink the socket file %s\n", opt->strarg);
            else
                logg(INFO, "Socket file removed.\n");
        }
#endif
    }
