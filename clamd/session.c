        ...
        if (!strncmp(cmd, commands[i].cmd, len)) {
            ...
            if (commands[i].need_arg) {
                if (!*arg) { /* missing argument */
                    logg(DEBUG_NV, "Command %s missing argument!\n", commands[i].cmd);
                    return COMMAND_UNKNOWN;
                }
                *argument = arg + 1;
            } else {
                if (*arg) { /* extra stuff after command */
                    logg(DEBUG_NV, "Command %s has trailing garbage!\n", commands[i].cmd);
                    return COMMAND_UNKNOWN;
                }
                *argument = NULL;
            }
            if (oldstyle && !commands[i].support_old) {
                logg(DEBUG_NV, "Command sent as old-style when not supported: %s\n", commands[i].cmd);
                return COMMAND_UNKNOWN;
            }
...
int command(client_conn_t *conn, int *virus)
{
    ...
    switch (conn->cmdtype) {
        ...
        case COMMAND_MULTISCAN: {
		   ...
		   make progress. */
                logg(WARNING, "Not enough threads for multiscan. Max: %d, Alive: %d, Multiscan: %d+1\n",
                     max, alive, multiscan);
        ...
        case COMMAND_ALLMATCHSCAN:
            if (!optget(opts, "AllowAllMatchScan")->enabled) {
                logg(DEBUG_NV, "Rejecting ALLMATCHSCAN command.\n");
            ...
            break;
        default:
            logg(ERROR, "Invalid command dispatched: %d\n", conn->cmdtype);
            return 1;
    }
    ...
    switch (cmd) {
        ...
        case COMMAND_ALLMATCHSCAN:
            ...
            if (!dup_conn->filename) {
                logg(ERROR, "Failed to allocate memory for filename\n");
                ret = -1;
            }
            break;
        case COMMAND_INSTREAMSCAN:
        ...
        case COMMAND_STATS:
            ...
            break;
        default:
            logg(ERROR, "Invalid command dispatch: %d\n", cmd);
            ret = -2;
            break;
    }
    if (!dup_conn->group)
    ...
    if (conn->group) {
        switch (cmd) {
            ...
            default:
                ...
                logg(DEBUG_NV, "SESSION: command is not valid inside IDSESSION: %d\n", cmd);
                conn->group = NULL;
                return 1;
        }
    }
