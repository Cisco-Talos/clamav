...
{
    logg(ERROR, "Localserver is not supported on this platform");
    return -1;
}
...
int localserver(const struct optstruct *opts)
{
    ...
    if (num_fd > 2) {
        logg(ERROR, "LOCAL: Received more than two file descriptors from systemd.\n");
        return -1;
    } else if (num_fd > 0) {
        ...
        if (sockfd == -2) {
            logg(INFO_NF, "LOCAL: No local AF_UNIX SOCK_STREAM socket received from systemd.\n");
            return -2;
        }
        logg(INFO_NF, "LOCAL: Received AF_UNIX SOCK_STREAM socket from systemd.\n");
        return sockfd;
    }
    /* create socket */
        ...
        if (stat(sockdir, &sb)) {
            if (errno == ENOENT) {
                ...
                if (mkdir(sockdir, sock_mode)) {
                    logg(ERROR, "LOCAL: Could not create socket directory: %s: %s\n", sockdir, strerror(errno));
                    if (errno == ENOENT) {
                        logg(ERROR, "LOCAL: Ensure parent directory exists.\n");
                    }
                } else {
                    logg(INFO, "Localserver: Creating socket directory: %s\n", sockdir);
                }
    ...
    if ((sockfd = socket(AF_UNIX, SOCK_STREAM, 0)) == -1) {
        estr = strerror(errno);
        logg(ERROR, "LOCAL: Socket allocation error: %s\n", estr);
        return -1;
    }
    ...
    if (bind(sockfd, (struct sockaddr *)&server, sizeof(struct sockaddr_un)) == -1) {
        if (errno == EADDRINUSE) {
            if (connect(sockfd, (struct sockaddr *)&server, sizeof(struct sockaddr_un)) >= 0) {
                logg(ERROR, "LOCAL: Socket file %s is in use by another process.\n", server.sun_path);
                close(sockfd);
                return -1;
            }
            if (optget(opts, "FixStaleSocket")->enabled) {
                logg(INFO_NF, "LOCAL: Removing stale socket file %s\n", server.sun_path);
                if (unlink(server.sun_path) == -1) {
                    estr = strerror(errno);
                    logg(ERROR, "LOCAL: Socket file %s could not be removed: %s\n", server.sun_path, estr);
                    close(sockfd);
                    return -1;
                }
                if (bind(sockfd, (struct sockaddr *)&server, sizeof(struct sockaddr_un)) == -1) {
                    estr = strerror(errno);
                    logg(ERROR, "LOCAL: Socket file %s could not be bound: %s (unlink tried)\n", server.sun_path, estr);
                    close(sockfd);
                    return -1;
                }
            } else if (CLAMSTAT(server.sun_path, &foo) != -1) {
                logg(ERROR, "LOCAL: Socket file %s exists. Either remove it, or configure a different one.\n", server.sun_path);
                close(sockfd);
                return -1;
            }
        } else {
            estr = strerror(errno);
            logg(ERROR, "LOCAL: Socket file %s could not be bound: %s\n", server.sun_path, estr);
            close(sockfd);
            return -1;
        }
    }

    logg(INFO_NF, "LOCAL: Unix socket file %s\n", server.sun_path);

    backlog = optget(opts, "MaxConnectionQueueLength")->numarg;
    logg(INFO_NF, "LOCAL: Setting connection queue length to %d\n", backlog);

    if (listen(sockfd, backlog) == -1) {
        estr = strerror(errno);
        logg(ERROR, "LOCAL: listen() error: %s\n", estr);
        close(sockfd);
        return -1;
    }
