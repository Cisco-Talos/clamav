...
dnsquery(const char *domain, int qtype, unsigned int *ttl)
{
    ...
    if (ttl)
        *ttl = 0;
    if (res_init() < 0) {
        logg(WARNING, "res_init failed\n");
        return NULL;
    }

    logg(DEBUG, "Querying %s\n", domain);
    ...
    if ((len = dn_expand(answer, answend, pt, host, sizeof(host))) < 0) {
        logg(WARNING, "dn_expand failed\n");
        return NULL;
    }

    pt += len;
    if (pt > answend - 4) {
        logg(WARNING, "Bad (too short) DNS reply\n");
        return NULL;
    }
    ...
    do { /* recurse through CNAME rr's */
        pt += size;
        if ((len = dn_expand(answer, answend, pt, host, sizeof(host))) < 0) {
            logg(WARNING, "second dn_expand failed\n");
            return NULL;
        }
        pt += len;
        if (pt > answend - 10) {
        ...
        if (pt + size < answer || pt + size > answend) {
            logg(WARNING, "DNS rr overflow\n");
            return NULL;
        }
    } while (type == T_CNAME);

    if (type != T_TXT) {
        logg(WARNING, "Not a TXT record\n");
        return NULL;
    }

    if (!size || (txtlen = *pt) >= size || !txtlen) {
        logg(WARNING, "Broken TXT record (txtlen = %d, size = %d)\n", txtlen, size);
        return NULL;
    }
