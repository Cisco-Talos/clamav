...
fc_error_t fc_initialize(fc_config *fcConfig)
{
    ...
    if (LSTAT(g_databaseDirectory, &statbuf) == -1) {
        logg(ERROR, "Database directory does not exist: %s\n", g_databaseDirectory);
        status = FC_EDIRECTORY;
        goto done;
    }
    if (!S_ISDIR(statbuf.st_mode)) {
        logg(ERROR, "Database directory is not a directory: %s\n", g_databaseDirectory);
        status = FC_EDIRECTORY;
        goto done;
    }
    ...
    if (FC_SUCCESS != load_freshclam_dat()) {
        logg(DEBUG, "Failed to load freshclam.dat; will create a new freshclam.dat\n");

        if (FC_SUCCESS != new_freshclam_dat()) {
            logg(WARNING, "Failed to create a new freshclam.dat!\n");
            status = FC_EINIT;
            goto done;
        }
    }
...
fc_error_t fc_prune_database_directory(char **databaseList, uint32_t nDatabases)
{
    ...
    if (!(dir = opendir(g_databaseDirectory))) {
        logg(ERROR, "checkdbdir: Can't open directory %s\n", g_databaseDirectory);
...
fc_error_t fc_test_database(const char *dbFilename, int bBytecodeEnabled)
{
    ...
    if ((NULL == dbFilename)) {
        logg(WARNING, "fc_test_database: Invalid arguments.\n");
        goto done;
    }

    logg(DEBUG, "Loading signatures from %s\n", dbFilename);
    ...
    if (CL_SUCCESS != (cl_ret = cl_load(
        ...
        logg(ERROR, "Failed to load new database: %s\n", cl_strerror(cl_ret));
        status = FC_ETESTFAIL;
        goto done;
    }
    ...
    if (bBytecodeEnabled && (CL_SUCCESS != (cl_ret = cli_bytecode_prepare2(
        ...
        logg(ERROR, "Failed to compile/load bytecode: %s\n", cl_strerror(cl_ret));
        status = FC_ETESTFAIL;
        goto done;
    }
    logg(DEBUG, "Properly loaded %u signatures from %s\n", newsigs, dbFilename);
    ...
    char **newVersion)
{
    ...
    if ((NULL == dnsUpdateInfo) || (NULL == newVersion)) {
        logg(WARNING, "dns_query_update_info: Invalid arguments.\n");
        status = FC_EARG;
        goto done;
    }
    ...
    *newVersion    = NULL;
    ...
    if (dnsUpdateInfoServer == NULL) {
        logg(WARNING, "DNS Update Info disabled. Falling back to HTTP mode.\n");
        goto done;
    }
    ...
    if (NULL == (reply_token = cli_strtok(dnsReply, DNS_UPDATEINFO_RECORDTIME, ":"))) {
        logg(WARNING, "Failed to find Record Time field in DNS Update Info.\n");
        goto done;
    }
    ...
    if (NULL == (reply_token = cli_strtok(dnsReply, DNS_UPDATEINFO_VERSIONWARNING, ":"))) {
        logg(WARNING, "Failed to find Version Warning Flag in DNS Update Info.\n");
        goto done;
    }
    ...
    if (NULL == (reply_token = cli_strtok(dnsReply, DNS_UPDATEINFO_NEWVERSION, ":"))) {
        logg(WARNING, "Failed to find New Version field in DNS Update Info.\n");
        goto done;
    }

    logg(DEBUG, "fc_dns_query_update_info: Software version from DNS: %s\n", reply_token);
    ...
    int *bUpdated)
{
    ...
    if ((NULL == database) || (NULL == serverList) || (NULL == bUpdated)) {
        logg(WARNING, "fc_update_database: Invalid arguments.\n");
        goto done;
    }
    ...
    for (i = 0; i < nServers; i++) {
        for (attempt = 1; attempt <= g_maxAttempts; attempt++) {
            ...
            switch (ret) {
                case FC_SUCCESS: {
                    if (*bUpdated) {
                        logg(DEBUG, "fc_update_database: %s updated.\n", dbFilename);
                    } else {
                        logg(DEBUG, "fc_update_database: %s already up-to-date.\n", dbFilename);
                    }
                    goto success;
                }
                case FC_ECONNECTION:
                ...
                case FC_EFAILEDGET: {
                    if (attempt < g_maxAttempts) {
                        logg(INFO, "Trying again in 5 secs...\n");
                        sleep(5);
                    } else {
                        logg(INFO, "Giving up on %s...\n", serverList[i]);
                        if (i == nServers - 1) {
                            logg(ERROR, "Update failed for database: %s\n", database);
                            status = ret;
                            goto done;
                        }
                    }
                ...
                case FC_EMIRRORNOTSYNC: {
                    logg(INFO, "Received an older %s CVD than was advertised. We'll retry so the incremental update will ensure we're up-to-date.\n", database);
                    break;
                }
                case FC_EFORBIDDEN: {
                    ...
                    logg(WARNING, "FreshClam received error code 403 from the ClamAV Content Delivery Network (CDN).\n");
                    logg(INFO, "This could mean several things:\n");
                    logg(INFO, " 1. You are running an out-of-date version of ClamAV / FreshClam.\n");
                    logg(INFO, "    Ensure you are the most updated version by visiting https://www.clamav.net/downloads\n");
                    logg(INFO, " 2. Your network is explicitly denied by the FreshClam CDN.\n");
                    logg(INFO, "    In order to rectify this please check that you are:\n");
                    logg(INFO, "   a. Running an up-to-date version of FreshClam\n");
                    logg(INFO, "   b. Running FreshClam no more than once an hour\n");
                    logg(INFO, "   c. If you have checked (a) and (b), please open a ticket at\n");
                    logg(INFO, "      https://github.com/Cisco-Talos/clamav/issues\n");
                    logg(INFO, "      and we will investigate why your network is blocked.\n");
                    logg(WARNING, "You are on cool-down until after: %s\n", retry_after_string);
                    status = ret;
                    goto done;
                    break;
                }
                case FC_ERETRYLATER: {
                    ...
                    strftime(retry_after_string, 26, "%Y-%m-%d %H:%M:%S", tm_info);
                    logg(WARNING, "FreshClam received error code 429 from the ClamAV Content Delivery Network (CDN).\n");
                    logg(INFO, "This means that you have been rate limited by the CDN.\n");
                    logg(INFO, " 1. Run FreshClam no more than once an hour to check for updates.\n");
                    logg(INFO, "    FreshClam should check DNS first to see if an update is needed.\n");
                    logg(INFO, " 2. If you have more than 10 hosts on your network attempting to download,\n");
                    logg(INFO, "    it is recommended that you set up a private mirror on your network using\n");
                    logg(INFO, "    cvdupdate (https://pypi.org/project/cvdupdate/) to save bandwidth on the\n");
                    logg(INFO, "    CDN and your own network.\n");
                    logg(INFO, " 3. Please do not open a ticket asking for an exemption from the rate limit,\n");
                    logg(INFO, "    it will not be granted.\n");
                    logg(WARNING, "You are on cool-down until after: %s\n", retry_after_string);
                    goto success;
                    break;
                }
                default: {
                    logg(ERROR, "Unexpected error when attempting to update %s: %s\n", database, fc_strerror(ret));
                    status = ret;
                    goto done;
                }
            }
    ...
    uint32_t *nUpdated)
{
    ...
    if ((NULL == databaseList) || (0 == nDatabases) || (NULL == serverList) || (NULL == nUpdated)) {
        logg(WARNING, "fc_update_databases: Invalid arguments.\n");
        goto done;
    }
    ...
    *nUpdated = 0;
    ...
    if (g_freshclamDat->retry_after > 0) {
        if (g_freshclamDat->retry_after > time(NULL)) {
            ...
            strftime(retry_after_string, 26, "%Y-%m-%d %H:%M:%S", tm_info);
            logg(WARNING, "FreshClam previously received error code 429 or 403 from the ClamAV Content Delivery Network (CDN).\n");
            logg(INFO, "This means that you have been rate limited or blocked by the CDN.\n");
            logg(INFO, " 1. Verify that you're running a supported ClamAV version.\n");
            logg(INFO, "    See https://docs.clamav.net/faq/faq-eol.html for details.\n");
            logg(INFO, " 2. Run FreshClam no more than once an hour to check for updates.\n");
            logg(INFO, "    FreshClam should check DNS first to see if an update is needed.\n");
            logg(INFO, " 3. If you have more than 10 hosts on your network attempting to download,\n");
            logg(INFO, "    it is recommended that you set up a private mirror on your network using\n");
            logg(INFO, "    cvdupdate (https://pypi.org/project/cvdupdate/) to save bandwidth on the\n");
            logg(INFO, "    CDN and your own network.\n");
            logg(INFO, " 4. Please do not open a ticket asking for an exemption from the rate limit,\n");
            logg(INFO, "    it will not be granted.\n");
            logg(WARNING, "You are still on cool-down until after: %s\n", retry_after_string);
            status = FC_SUCCESS;
            goto done;
        } else {
            g_freshclamDat->retry_after = 0;
            logg(WARNING, "Cool-down expired, ok to try again.\n");
            save_freshclam_dat();
        }
    }
    ...
    int *bUpdated)
{
    ...
    if ((NULL == urlDatabase) || (NULL == bUpdated)) {
        logg(WARNING, "fc_download_url_database: Invalid arguments.\n");
        goto done;
    }
    ...
    *bUpdated = 0;
    ...
    for (attempt = 1; attempt <= g_maxAttempts; attempt++) {
        ...
        switch (ret) {
            case FC_SUCCESS: {
                if (*bUpdated) {
                    logg(DEBUG, "fc_download_url_database: %s updated.\n", dbFilename);
                } else {
                    logg(DEBUG, "fc_download_url_database: %s already up-to-date.\n", dbFilename);
                }
                goto success;
            }
            case FC_ECONNECTION:
            ...
            case FC_EFAILEDGET: {
                if (attempt < g_maxAttempts) {
                    ...
                    logg(INFO, "Update failed for custom database URL: %s\n", urlDatabase);
                    status = ret;
                    goto done;
                }
                break;
            ...
            default: {
                logg(INFO, "Unexpected error when attempting to update from custom database URL: %s\n", urlDatabase);
                status = ret;
                goto done;
            }
        }
    ...
    uint32_t *nUpdated)
{
    ...
    if ((NULL == urlDatabaseList) || (0 == nUrlDatabases) || (NULL == nUpdated)) {
        logg(WARNING, "fc_download_url_databases: Invalid arguments.\n");
        goto done;
    }
    ...
    *nUpdated = 0;
    ...
    for (i = 0; i < nUrlDatabases; i++) {
        if (FC_SUCCESS != (ret = fc_download_url_database(
                               ...
                               context,
                               &bUpdated))) {
            logg(WARNING, "fc_download_url_databases: fc_download_url_database failed: %s (%d)\n", fc_strerror(ret), ret);
            status = ret;
            goto done;
        }
        if (bUpdated) {
