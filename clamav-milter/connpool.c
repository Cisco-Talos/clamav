...
static int cpool_addunix(char *path)
{
    ...
    if (!cli_is_abspath(path)) {
        logg(ERROR, "Unix clamd socket must be an absolute path\n");
        return 1;
    }
    if (!(srv = (struct sockaddr_un *)malloc(sizeof(*srv)))) {
        logg(ERROR, "Out of memory allocating unix socket space\n");
        return 1;
    }
    ...
    logg(DEBUG, "Local socket unix:%s added to the pool (slot %d)\n", srv->sun_path, cp->entries);
    return 0;
}
...
static int cpool_addtcp(char *addr, char *port)
{
    ...
    if (getaddrinfo(addr, port ? port : "3310", &hints, &res)) {
        logg(WARNING, "Can't resolve hostname %s\n", addr ? addr : "");
        return 1;
    }
    cpe->type = 1;
    cpe->dead = 1;
...
static int addslot(void)
{
    ...
    if (!(cpe = realloc(cp->pool, (cp->entries + 1) * sizeof(struct CP_ENTRY)))) {
        logg(ERROR, "Out of memory while initializing the connection pool\n");
        cpool_free();
        return 1;
    }
    if (cp->local_cpe)
...
static void cpool_probe(void)
{
    ...
    for (i = 1; i <= cp->entries; i++) {
        if ((cpe->dead && (cpe->last_poll < now - 120 || !cp->alive)) || cpe->last_poll < now - 15 * 60 * 60) {
            ...
            nc_ping_entry(cpe);
            logg(DEBUG, "Probe for slot %u returned: %s\n", i, cpe->dead ? "failed" : "success");
        }
        dead += cpe->dead;
        cpe++;
    }
    ...
    if (!cp->alive)
        logg(WARNING, "No clamd server appears to be available\n");
}
...
void cpool_init(struct optstruct *opts)
{
    ...
    if ((opt = optget(opts, "ClamdSocket"))->enabled) {
        while (opt) {
            ...
            if (!strncasecmp(socktype, "unix:", 5)) {
                ...
                logg(ERROR, "Failed to parse ClamdSocket directive '%s'\n", socktype);
                failed = 1;
            }
            if (failed) break;
            opt = opt->nextarg;
    ...
    if (!cp->entries) {
        logg(ERROR, "No ClamdSocket specified\n");
        cpool_free();
        return;
    }
    quitting = 0;
...
void cpool_free(void)
{
    ...
    if (!quitting) {
        logg(DEBUG, "Killing the monitor and stopping\n");
        quitting = 1;
