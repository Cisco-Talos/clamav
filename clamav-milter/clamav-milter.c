...
static void milter_exit(int sig)
{
    ...
    logg(DEBUG, "clamav-milter: milter_exit, signal %d\n", sig);
    ...
    logg(INFO, "clamav-milter: stopped\n");
...
int main(int argc, char **argv)
{
    ...
    if ((opt = optget(opts, "Chroot"))->enabled) {
        if (chdir(opt->strarg) != 0) {
            logg(ERROR, "Cannot change directory to %s\n", opt->strarg);
            return 1;
        }
        if (chroot(opt->strarg) != 0) {
            logg(ERROR, "chroot to %s failed. Are you root?\n", opt->strarg);
            return 1;
        }
    }
    ...
    if (!(my_socket = optget(opts, "MilterSocket")->strarg)) {
        logg(ERROR, "Please configure the MilterSocket directive\n");
        logg_close();
        optfree(opts);
        return 1;
    }
    ...
    if (smfi_setconn(my_socket) == MI_FAILURE) {
        logg(ERROR, "smfi_setconn failed\n");
        logg_close();
        optfree(opts);
        return 1;
    }
    if (smfi_register(descr) == MI_FAILURE) {
        logg(ERROR, "smfi_register failed\n");
        logg_close();
        optfree(opts);
        return 1;
    }
    ...
    if (smfi_opensocket(opt->enabled) == MI_FAILURE) {
        logg(ERROR, "Failed to create socket %s\n", my_socket);
        logg_close();
        optfree(opts);
        return 1;
    }
    ...
    if (strncmp(my_socket, "inet:", 5) && strncmp(my_socket, "inet6:", 6)) {
        ...
        if (NULL != user_name) {
            ...
            if ((user = getpwnam(user_name)) == NULL) {
                logg(INFO, "ERROR: Can't get information about user %s.\n",
                     user_name);
                logg_close();
                optfree(opts);
                return 1;
            ...
            if (chown(sock_name, user->pw_uid, -1)) {
                logg(ERROR, "Failed to change socket ownership to user %s\n", user->pw_name);
                optfree(opts);
                logg_close();
                return 1;
            }
        ...
        if (optget(opts, "MilterSocketMode")->enabled) {
            ...
            if (*end) {
                logg(ERROR, "Invalid MilterSocketMode %s\n", optget(opts, "MilterSocketMode")->strarg);
                logg_close();
                optfree(opts);
                return 1;
            }
        ...
        if (chmod(sock_name, sock_mode & 0666)) {
            logg(ERROR, "Cannot set milter socket permission to %s\n", optget(opts, "MilterSocketMode")->strarg);
            logg_close();
            optfree(opts);
            return 1;
        }
    ...
    time(&currtime);
    if (logg(INFO_NF, "+++ Started at %s", ctime(&currtime))) {
    ...
    if (!maxfilesize) {
        logg(WARNING, "Invalid MaxFileSize, using default (%d)\n", CLI_DEFAULT_MAXFILESIZE);
    ...
    cpool_init(opts);
    if (!cp) {
        logg(ERROR, "Failed to init the socket pool\n");
        localnets_free();
        allow_list_free();
        logg_close();
        optfree(opts);
