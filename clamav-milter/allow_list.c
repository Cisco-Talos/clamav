...
int allow_list_init(const char *fname)
{
    ...
    if (!(f = fopen(fname, "r"))) {
        logg(ERROR, "Cannot open allow list file '%s'\n", fname);
        return 1;
    }
    ...
    while (fgets(buf, sizeof(buf), f) != NULL) {
        ...
        if (!(w = (struct WHLST *)malloc(sizeof(*w)))) {
            logg(ERROR, "Out of memory loading allow list file\n");
            allow_list_free();
            fclose(f);
            return 1;
        }
        ...
        if (cli_regcomp(&w->preg, ptr, REG_ICASE | REG_NOSUB)) {
            logg(ERROR, "Failed to compile regex '%s' in allow list file\n", ptr);
            allow_list_free();
            fclose(f);
            return 1;
        }
...
int smtpauth_init(const char *r)
{
    ...
    if (!strncmp(r, "file:", 5)) {
        ...
        while (fgets(buf, sizeof(buf), f) != NULL) {
            ...
            if (len * 3 + 1 > rxavail) {
                ...
                if (!regex) {
                    logg(ERROR, "Cannot allocate memory for SkipAuthenticated file\n");
                    fclose(f);
                    return 1;
                }
                rxavail = 2048;
    ...
    if (cli_regcomp(&authreg, r, REG_ICASE | REG_NOSUB | REG_EXTENDED)) {
        logg(ERROR, "Failed to compile regex '%s' for SkipAuthenticated\n", r);
        if (regex) free(regex);
        return 1;
    }
    if (regex) free(regex);
